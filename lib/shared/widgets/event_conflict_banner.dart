...
/// стало приглашением.
///
/// --- ТЕЛО ПЕРЕЕХАЛО 12.08 В `core/agreements/day_role.dart` (N126) ---
///
/// **Здесь осталась обёртка, и это не косметика.** Сетка месяца задавала тот
/// же вопрос и отвечала на него **своим способом** — точнее, не отвечала
/// вовсе: она красила день по `ownerUid`, не читая ответ, отчего приглашение
/// и отказ помечали день занятым (N126). Правило нельзя было позвать оттуда:
/// оно живёт в `shared/widgets`, а сетка — в `core`, и ссылка из `core` на
/// виджеты запрещена слоями.
///
/// Поэтому правило переехало в `core` целиком, вместе со всеми доводами
/// (пустой uid, владелец, `notAsked`, старые документы), а **это имя
/// сохранено обёрткой** — у него один вызывающий, разбор конфликтов, и
/// вопрос там двоичный: мешает или не мешает. Переименовать его заодно
/// значило бы смешать две правки в одну.
///
/// **Двоичность — вся разница между этими двумя именами.** `dayRoleOf`
/// отвечает тремя значениями, потому что показу нужны три; здесь третье
/// («приглашён») сливается со «свободен» — и это ЗАКОННО ровно потому, что
/// приглашение конфликтом не является: оно не мешает человеку пойти в другое
/// место. Разбор трёх значений и все границы — там же, в `day_role.dart`.
bool occupiesCalendarOf(PersonalEvent e, String uid) =>
    dayRoleOf(e, uid) == DayRole.occupied;
...
/// **ВЫЗЫВАЮЩИХ В `lib` СЕЙЧАС НОЛЬ, И ЭТО СКАЗАНО ЗДЕСЬ, А НЕ ВЫЯСНИТСЯ
/// ПОТОМ** (`grep -rn "participantsAfterConflictChange" lib` → одно
/// объявление и ни одного вызова, 30.08).
///
/// Единственный вызывающий — колесо даты в `agreements_screen.dart` — снят
/// починкой N187: эта функция делает два дела разом, снимает подмешанных
/// прошлого конфликта И подмешивает состав нового, а второе и есть
/// подмешивание до ответа. На колесе осталась только снимающая половина,
/// `participantsAfterUnmerge`.
///
/// **НЕ УДАЛЕНА НАМЕРЕННО, и довод не в бережливости.** Семь тестов на неё —
/// это записанный разбор N38 (состав налипал при прокрутке даты через
/// занятую минуту, наблюдалось дважды, 04.08 и 06.08). Удали функцию — и
/// уйдёт знание, а не код. Понадобится она снова в тот день, когда
/// подмешивание вернут на любой путь ДО ответа; до тех пор здесь стоит эта
/// пометка, чтобы никто не считал её живой.
...
List<PersonalEvent> conflictEventsOnDay(
  ...
  for (final e in events) {
    ...
    // ПРИГЛАШЕНИЯ СВОИХ НЕ ПОВТОРЯЮТ КОНФЛИКТ (N211, 09.09).
    //
    // Позвал троих на пятницу — и, заводя на ту же пятницу другой вечер,
    // человек получал ЧЕТЫРЕ конфликта об одном и том же деле.
    //
    // НАСТОЯЩИЙ КОНФЛИКТ ПРИ ЭТОМ НЕ ТЕРЯЕТСЯ, и это проверяемо: родитель
    // стоит в том же перечне, на том же дне и в тот же час, и он не
    // схлопывается — схлопываются только его дети. То есть предупреждение
    // остаётся, исчезают повторы (I14: назвать, каким был бы вывод на
    // исправном, — здесь он «один конфликт», а не «ни одного»).
    //
    // ФИЛЬТР СТОИТ ЗДЕСЬ, А НЕ В `occupiesCalendarOf`, НАРОЧНО. Тот отвечает
    // на двоичный вопрос «мешает ли», и ответ у ребёнка честный — мешает,
    // это занятый вечер. Здесь же перечисление, а повтор мешает не дважды
    // (I58: разделять по задаче, а не по совпадению вида).
    if (collapsesUnderParent(e, currentUid)) continue;
