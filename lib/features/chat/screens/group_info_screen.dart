...
class _AddParticipantsSheetState extends ConsumerState<_AddParticipantsSheet> {
  ...
  late final UserSearchController _searchCtrl = UserSearchController(
    filters: _filters.toAlgoliaFiltersExcluding(widget.existingMembers),
  );
  ...
  Future<void> _openFilters() async {
    ...
    if (result != null) {
      ...
      _searchCtrl.updateFilters(
        _filters.toAlgoliaFiltersExcluding(widget.existingMembers),
      );
    }
  }
                        ...
                        leading: SizedBox(
                          ...
                          child: Stack(
                            ...
                            children: [
                              ...
                              // `border: null`.
                              // `adminUid` — ЭТО СМОТРЯЩИЙ, И ИМЯ ОБ ЭТОМ НЕ
                              // ГОВОРИТ (проверено 19.09, обход одиннадцати
                              // мест ободка).
                              //
                              // Все четыре вызова `_AddParticipantsSheet`
                              // (:95, :164, :207, :255) подают `adminUid:
                              // _myUid`, а `_myUid` — это
                              // `FirebaseAuth.currentUser.uid` (:47). То есть
                              // значение верное: лист открывает админ, админ и
                              // есть смотрящий.
                              //
                              // ОГОВОРКА СТОИТ ЗДЕСЬ, ПОТОМУ ЧТО ИМЯ ДЕЛАЕТ
                              // ВЕРНОСТЬ НЕВИДИМОЙ. Ободок спрашивает «я ли
                              // это» и «смотрел ли Я» — вопросы про
                              // СМОТРЯЩЕГО. Подставь сюда однажды настоящего
                              // админа, отличного от смотрящего, и ободок
                              // станет отвечать на его вопрос вместо моего:
                              // чужая непросмотренная история погаснет, потому
                              // что её смотрел админ. Молча — ни один сторож
                              // на это не смотрит.
                              StatusRing(
                                user: u,
