...
class _ChatScreenState extends ConsumerState<ChatScreen>
  ...
  // ПРЯМОЙ ЛИ ЭТО ЧАТ. Снимается в build() вместе с `_otherUserCached` выше и
  // существует только ради него — точнее, ради одного его читателя,
  // `_replySenderName`.
  //
  // `_otherUserCached` берётся по `otherUidResolved`, а тот в ГРУППЕ — первый
  // попавшийся участник, не я (оговорка стоит у самого `otherUids`). Шапке,
  // звонкам и «печатает» этого хватает: они живут только в прямом чате. Имени
  // АВТОРА ЦИТАТЫ — не хватает: в группе цитируемое написал кто угодно, и
  // назвать его первым попавшимся значит записать в данные УВЕРЕННУЮ неправду
  // вместо неуверенной нелепости. Поэтому в группе имени нет, и так и сказано.
  bool _isDirectChatCached = false;
  ...
  /// ИМЯ АВТОРА ЦИТИРУЕМОГО СООБЩЕНИЯ. Источник — живой пользователь.
  ///
  /// **ПОЧИНЕНО 25.08 (N141).** Стояло `chatData['name']` — поле документа
  /// чата, названное негодным в этом же файле у заголовка (искать по
  /// `otherUidResolved`, где присвоение `_otherUserCached` и стоит):
  /// **mugam-v2 пишет его со стороны СОЗДАТЕЛЯ чата и никогда не обновляет**,
  /// поэтому вторая сторона читает там своё собственное имя, а в чатах нового
  /// приложения — пустоту.
  ///
  /// **Цена здесь была выше, чем у показа: имя ЗАПИСЫВАЕТСЯ в документ
  /// сообщения** (`replyTo.senderName`), то есть неправда закреплялась в
  /// данных навсегда, а не только рисовалась на экране. Перепись прода 25.08:
  /// **10 цитат, у одной имя расходится** («Q1» против «Rafael Dagli»); пустых
  /// имён **ноль**. Лечения задним числом нет и не планируется: единственную
  /// расходящуюся цитату **нечем отличить от переименования человека** (N142),
  /// а проход по `chats/*/messages` необратим.
  ///
  /// **ТРЕТЬЯ ВЕТВЬ — ПУСТОТА В ГРУППЕ, И ОНА ЗДЕСЬ ГЛАВНАЯ.** `_otherUserCached`
  /// в группе — первый попавшийся участник (см. `_isDirectChatCached`), а
  /// цитируемое там мог написать кто угодно. Подписать чужой цитатой живого
  /// человека значило бы завести ту же беду, ради которой правка и делалась,
  /// только УВЕРЕННУЮ: имя группы читается как нелепость и не обманывает,
  /// имя соседа — обманывает и уезжает в данные. Поэтому в группе — «не
  /// знаем», а не догадка (I47).
  ///
  /// Сторожится по исходникам: `source_invariants_test.dart`, «е».
  String _replySenderName(Message msg, String currentUid) {
    if (msg.senderId == currentUid) {
      return FirebaseAuth.instance.currentUser?.displayName ?? '';
    }
    return _isDirectChatCached ? (_otherUserCached?.name ?? '') : '';
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Признак «собеседник ровно один» — снимается здесь же, из `otherUids`,
    // а не из `isGroup` документа чата: `otherUids` и есть то, по чему выше
    // выбран `otherUidResolved`, то есть тот самый список, чью длину и надо
    // знать. Поле `isGroup` отвечало бы на соседний вопрос (I13).
    _isDirectChatCached = otherUids.length == 1;
    return Scaffold(
      appBar: AppBar(
        backgroundColor: kBg2,
        elevation: 0,
