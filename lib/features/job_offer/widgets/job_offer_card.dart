...
class _JobOfferCardState extends State<JobOfferCard> {
  ...
  /// ДВА ЧИСЛА, А НЕ ОДНО, и в этом вся суть настройки (решение автора
  /// 14.08).
  ///
  /// Сворачиваем, только когда прячется что-то ЗАМЕТНОЕ, и показываем при
  /// этом меньше порога. Одно общее число дало бы «yenə 1 gün»: список из
  /// семи при пороге шесть свернулся бы ради одной спрятанной строки —
  /// человек нажимает, чтобы увидеть один день. Экономия должна быть
  /// настоящей, иначе сворачивание только мешает.
  ///
  /// Отсюда: свернуть при БОЛЬШЕ ВОСЬМИ, показать ПЯТЬ. Девять дней прячут
  /// четыре, тридцать — двадцать пять. Меньшее «yenə» из возможных — 4.
  ///
  /// На телефоне числа НЕ проверялись (I50). Подвинуть — правка двух строк.
  static const int _collapseAbove = 8;
  static const int _collapseShow = 5;
  ...
  List<Widget> _dayRows(JobOffer offer, bool canPick) {
    ...
    //
    // СВОЙСТВО МЕСТА, А НЕ РОЛИ — и переписывать это на роль НЕЛЬЗЯ.
    //
    // Формулировка автора была «свёрнуто у отправителя, развёрнуто у
    // отвечающего», и она верна ровно для двух сегодняшних сторон. Условие
    // же написано не про них: `!canPick` — «список нельзя тронуть».
    //
    // Разница проявится на ТРЕТЬЕМ случае, которого сегодня нет. Роль
    // «отправитель» ничего не говорит о закрытом раунде: после принятия
    // отвечавший тоже ничего тронуть не может, и по роли ему полагался бы
    // развёрнутый список — тридцать строк под уже состоявшейся сделкой.
    // По месту он свернётся сам, без правки этого файла.
    //
    // И наоборот: появись состояние, где отметки живые у кого-то ещё, он
    // получит полный список по устройству, а не потому, что кто-то вспомнил
    // дописать его роль в условие.
    //
    // ПРАВИЛО ДЛЯ СЛЕДУЮЩЕГО: если тянет заменить `!canPick` на проверку
    // роли (`isInitiator`, `uid == createdBy`) — не заменять. Условие
    // спрашивает про СПИСОК, и спрашивать оно должно именно про него.
    final collapsible = !canPick && all.length > _collapseAbove;
    final shown = collapsible && !_daysExpanded
        ? all.take(_collapseShow).toList()
        : all;
    _hiddenDaysCount = collapsible && !_daysExpanded
        ? all.length - _collapseShow
        : 0;
