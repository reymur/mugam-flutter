...
class _JobOfferCardState extends State<JobOfferCard> {
  ...
  Widget _footer(JobOffer offer, OfferCardActions actions, bool isInitiator) {
    ...
    if (actions.canAnswer && widget.onOpenAnswer != null) {
      ...
      // `!hasAnswered` ДОПИСАНО 20.08 — ОБРАТНАЯ СТОРОНА ПРАВИЛА «НЕ
      // РИСОВАТЬ КНОПКУ БЕЗ АДРЕСАТА», А НЕ ЕГО ПОВТОР (N154).
      //
    ...
    return Column(
      ...
      children: [
        ...
        //
        // Это обратная сторона правила «не рисовать кнопку без адресата»
        // (N146/N154): там нажимаемое без дела, здесь дело без вида. Первую
        // сторону искали дважды, вторую — ни разу; перепись — N174.
        //
