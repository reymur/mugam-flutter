...
class _JobOfferCardState extends State<JobOfferCard> {
  ...
  Widget _footer(JobOffer offer, OfferCardActions actions, bool isInitiator) {
    ...
    // НЕГО ВЫПАЛ. Правило «кнопка без адресата не рисуется» записано в этом
    // файле дважды словами (у `onOpenAnswer` и у отзыва в конце `_footer`) и
    // соблюдено у ответа, приёма и отзыва; здесь стояло голое
    // `if (actions.canRecordVoice)`, то есть кружок рисовался всегда, а
    // `onTap` при неподключённой записи был `null`.
    //
    // Видно это стало на телефоне: у получателя на листе предложения
    // микрофон оказался ЕДИНСТВЕННЫМ нажимаемым местом и не делал ничего.
    //
    // Проверять надо было не наличие правила в файле, а КАЖДОГО, кто под
    // него подпадает: правило было записано, соблюдено у трёх из четырёх, и
    // четвёртый промолчал.
    if (actions.canRecordVoice && widget.onRecordVoice != null) {
      children.add(
        InkWell(
      ...
      children.add(
        Expanded(
          child: _goldButton(
            ...
            // «CAVABA BAX», А НЕ «QƏBUL EDİRƏM», И ЭТО НЕ ПРИДИРКА К СЛОВУ.
            //
            // Кнопка ОТКРЫВАЕТ ЭКРАН приёма, а принимают уже на нём —
            // там «Qəbul edirəm» и стоит. Назвать обе одинаково значило бы
            // пообещать действие и показать вместо него новый экран.
            //
            // Ровно та же правка и по той же причине была сделана на шаге
            // 1: под квадратиками в макетах стояло «Göndər» (оно
            // ЗАВЕРШАЛО ход), а кнопка, открывающая лист ответа, названа
            // «Cavab ver». Здесь пара такая же: смотрю ответ → принимаю.
            label: 'Cavaba bax',
            onTap: widget.onAccept,
          ),
        ),
      );
    } else if (isInitiator && !offer.hasAnswered(widget.recipientUid)) {
      // `!hasAnswered` ДОПИСАНО 20.08 — ПРОДОЛЖЕНИЕ ПРАВИЛА «НЕ РИСОВАТЬ
      // КНОПКУ БЕЗ АДРЕСАТА», А НЕ ЕГО ПОВТОР (N154): кнопку сняли верно, а
      // спросить забыли, ЧТО встало на её место.
      //
      // НЕ ПУТАТЬ С N174 в тридцати строках ниже, у отзыва: там про то, как
      // выглядит САМ существующий ход, здесь — про то, что экран говорит
      // ВМЕСТО снятого. Обе 27.08 звались «обратной стороной» одного правила,
      // и обе переименованы, чтобы их не сочли дублем.
      //
      // Условие веткой выше (`canAccept && onAccept != null`) сужено 19.08
      // нарочно, чтобы после ответа не рисовалась мёртвая «Qəbul edirəm»:
      // приём подключается шагом 3. Сужение своё дело сделало — и **тем же
      // движением открыло дорогу сюда**. У инициатора после ответа
      // `canAccept` истинен, `onAccept` пуст, все ветви выше промахиваются,
      // и он падал в этот `else`.
      //
      // А этот `else` не пустой: он УТВЕРЖДАЕТ. Рафаэль видел «Teymur
      // Orucovdan cavab gözlənilir» **под шапкой «TEYMUR ORUCOV CAVAB
      // VERDİ» и крупным «0 gün»** — экран спорил сам с собой. Найдено на
      // трубке 20.08, в первый же прогон после подключения шага 2: до него
      // состояние «ответили» в приложении было недостижимо.
      //
      // **«Кнопки нет» безопасно, «строка врёт» — нет.** Отсюда и признак:
      // сузив условие, спросить не только что перестало рисоваться, но и
      // КУДА ТЕПЕРЬ ПАДАЕТ тот, кого условие не поймало, — и если там
      // `else`, проверить, что он утверждает.
      //
      // МОЛЧАНИЕ, А НЕ ДРУГАЯ СТРОКА (решение владельца 20.08): правда уже
      // написана трижды — шапка, «0 gün», строка отказанных дней. Строка
      // «принять пока нельзя» объясняла бы человеку наше расписание работ,
      // а не его дело.
      children.add(
        Expanded(
          child: Text(
    ...
    return Column(
      ...
      children: [
        ...
        // ВИД ДЕЙСТВИЯ, А НЕ ПРОДОЛЖЕНИЕ СООБЩЕНИЯ (N174, 27.08).
        //
        // Здесь стояло `kMuted, fontSize: 12`. Строка состояния прямо над
        // этой («…dan cavab gözlənilir») — `kMuted, fontSize: 13`, то есть
        // ТОТ ЖЕ ЦВЕТ И ТО ЖЕ НАЧЕРТАНИЕ, а различие было ровно в одном
        // пункте кегля. И работало оно в минус: действие оказывалось МЕЛЬЧЕ
        // состояния, а меньший кегль читается как сноска. Владелец на трубке
        // прочитал отзыв как продолжение сообщения и не как ход.
        //
        // У НАЖИМАЕМОГО ДВЕ ПОЛОВИНЫ — АДРЕСАТ И ВИД. Правило «не рисовать
        // кнопку без адресата» (N146) стережёт первую: там нажимаемое без
        // дела, здесь дело без вида. Первую искали дважды, вторую — ни разу;
        // перепись — N174.
        //
        // НЕ ПУТАТЬ С N154 в тридцати строках выше, у ожидания ответа: она
        // про то, что экран говорит ВМЕСТО снятого хода, эта — про то, как
        // выглядит САМ ход. Обе 27.08 звались «обратной стороной» одного
        // правила, и обе переименованы, чтобы их не сочли дублем.
        //
        // ЗОЛОТОЙ ТЕКСТ БЕЗ ЗАЛИВКИ — ЯРУС, КОТОРЫЙ В ПРОЕКТЕ УЖЕ ЕСТЬ, а не
        // третья форма:
        //   заливка `kGold` + `kOnGold`  — главный ход (`_goldButton` ниже);
        //   текст `kGold` без заливки    — второстепенный («Ətraflı»,
        //                                  `job_offer_days_sheet.dart:576`);
        //   текст `kMuted`               — закрыть/назад («Bağla», «← …»).
        // Отзыв встаёт вторым ярусом: от состояния его отличает ЦВЕТ (а не
        // кегль, который уже пробовали), от «Qəbul edirəm» — отсутствие
        // заливки. Необратимость выражена именно этим: ход виден, но не
        // зазывает.
        //
        // НЕ `kRed`: красный в проекте занят удалением объекта в ряду
        // диалога («Sil», «Çıx»), а отзыв закрывает ход, а не удаляет запись.
        // НЕ подчёркивание: `TextDecoration` не встречается в `lib` ни разу
        // (`grep -rn "TextDecoration" lib` → пусто), и одно место стало бы
        // единственным носителем приёма.
        // НЕ рамка: рамка занята входным действием (`home_screen.dart:93`).
        //
        // КЕГЛЬ 14 — ОБЩИЙ С `offer_answer_view.dart`. До сегодня было 12
        // здесь и 14 там; расхождение никто не замечал, потому что рядом эти
        // два места не видно никогда.
        InkWell(
