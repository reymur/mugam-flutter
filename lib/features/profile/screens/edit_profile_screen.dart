...
class _EditProfileScreenState extends ConsumerState<EditProfileScreen> {
  ...
  // ЕДИНСТВЕННАЯ ВЕТВЬ, НЕ СВЁРНУТАЯ В `AvatarRing` — И ЭТО РЕШЕНИЕ, А НЕ
  // НЕДОСМОТР (свёртка 16.09).
  //
  // Здесь не показ чужого аватара, а ВЫБОР своего: источник картинки
  // выбирается между `FileImage` (снимок, только что положенный на диск и
  // ещё не загруженный) и `NetworkImage` (уже сохранённый), а поверх лежит
  // значок камеры. `AvatarRing` не умеет ни локального файла, ни значка, и
  // учить его этому значило бы влить в него вторую задачу — ровно то, что
  // запрещает I58: одинаковый вид не есть одинаковое дело.
  //
  // Под подписанные ссылки это место придётся тронуть отдельно и по-своему:
  // у него ДВА источника, и заменять надо только второй.
