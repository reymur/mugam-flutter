...
class DayScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final buckets = buildDayBuckets(
      ...
      now: now,
      // ТОТ ЖЕ `uid`, ЧТО У ОБОИХ ПОТОКОВ ВЫШЕ, а не снятый заново. Возьми
      // его вторым вызовом `FirebaseAuth.instance` — и раскладка считала бы
      // про одного человека по спискам другого; сегодня они совпали бы, но
