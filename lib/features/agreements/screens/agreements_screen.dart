                    ...
                    if (event.participantUids.isNotEmpty) ...[
                      ...
                      for (final uid in event.participantUids)
                        _PartyMemberRow(
                          ...
                          // `left` не писало ни одно место в `lib` (N121), и
                          // условие показа кончалось на `answer == left`.
                          //
                          // ПИСАТЕЛЬ ПОЯВИЛСЯ 29.08 (шаг 2), И РЕШЕНИЕ ОТ
                          // ЭТОГО НЕ ПЕРЕМЕНИЛОСЬ. Довод 13.08 был не «нам
                          // нечего показать», а «показывать незачем»:
                          // вышедший помечен «İşdən çıxdı», спросить можно в
                          // чате, а окошко предлагало дорогу, которая и так
                          // есть. Второй половины довода — «показать всё
                          // равно некому» — с сегодня нет, первая цела.
                          // Вернуть кнопку — отдельное решение владельца, а
                          // не следствие писателя (N179: снятый показ —
                          // след решения, а не забытый код).
                          onAsk: null,
                        ),
                    ],
