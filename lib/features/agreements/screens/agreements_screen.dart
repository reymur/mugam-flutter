              ...
              child: IntrinsicHeight(
                child: Column(
                  ...
                  children: [
                    ...
                    // С 17.09 ДРУГОЙ, ХОТЯ РЕШЕНИЕ ТО ЖЕ.
                    //
                    // ЗДЕСЬ СТОЯЛО: «состав вечера — это ЭТОТ документ, а
                    // позванные живут в ДРУГИХ документах, своих у каждого».
                    // Это перестало быть правдой: позванный теперь в составе
                    // ЭТОГО документа, и разделять их по месту жительства
                    // нечем.
                    //
                    // РАЗДЕЛЯЕТ ИХ ВОПРОС, А НЕ МЕСТО (I58: по задаче, а не по
                    // совпадению). Список выше отвечает «кто идёт на вечер» —
                    // там все, включая вписанных руками. Этот отвечает «кого Я
                    // ПОЗВАЛ и чем это кончилось» — и только он знает про
                    // НЕПОЗВАННОГО, у которого ни строки в составе, ни ответа.
                    // Поэтому правило ходит по шаблону, а не по составу, и
                    // вписанный руками сюда не попадает — ровно как не попадал
                    // прежде, когда у него не заводилось ребёнка.
                    //
                    // ТОЛЬКО ВЛАДЕЛЬЦУ — И ЭТО ТЕПЕРЬ РЕШЕНИЕ ПОКАЗА, А НЕ
                    // СЛЕДСТВИЕ ПРАВ. Прежний довод «правило чтения не пустит
                    // его к чужим приглашениям вовсе» СНЯТ КАК НЕВЕРНЫЙ:
                    // приглашённый состоит в `musicians` этого вечера, значит
                    // `isParty()` пускает его к документу целиком, шаблон
                    // включая. Скрывает раздел эта строка и только она.
                    // Владелец знал о расширении видимости до работы и
                    // согласился (решение 17.09); если когда-нибудь понадобится
                    // скрыть шаблон по-настоящему — это отдельный документ, а
                    // не условие здесь (правилами поля не закрыть, N236).
                    if (isOwner) ...[
                      Builder(builder: (context) {
                        final rows = lineupRows(
                          lineup: event.lineup,
                          // САМ ВЕЧЕР, А НЕ СПИСОК ЧУЖИХ ДОКУМЕНТОВ. Здесь
                          // стояло `children: personalEvents` — правило искало
                          // среди них детей по `parentEventId`. Приглашение
                          // живёт в составе этого же вечера (17.09), и
                          // спрашивать больше нечего и некого.
                          event: event,
