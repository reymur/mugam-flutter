...
// `lineup_rows.dart` остаётся ради ОДНОЙ функции — `invitationsFollowing`:
// она рассылает выбранное владельцем состояние по документам-приглашениям.
// Приглашений больше не создают, но девять штук живут в проде до общего
// стирания, и правило обязано с ними работать. Уйдёт вместе с параметром
// `alsoIds` у `setEventStatus`, отдельным шагом.
import '../../../core/agreements/lineup_rows.dart';
import '../../../core/agreements/party_rows.dart';
...
class _MyAnswerCardState extends State<_MyAnswerCard> {
  ...
  /// ОТКАЗ С ПРИЧИНОЙ — ЕДИНСТВЕННЫЙ СПОСОБ СКАЗАТЬ «НЕТ» (17.09).
  ///
  /// **КОНФЛИКТ ЗДЕСЬ НЕ СПРАШИВАЕТСЯ, И ЭТО НЕ ЗАБЫВЧИВОСТЬ.** Окно конфликта
  /// предупреждает того, кто ЗАНИМАЕТ время; отказывающийся его освобождает, и
  /// мешать ему нечем. Довод перенесён с прежней кнопки без изменений.
  ///
  /// **ТЕЛО НЕ ЗДЕСЬ, А В `leaveEventWithReason`:** дверей в отказ стало две —
  /// карточка вечера и экран приглашения, — и ход у них обязан быть один.
  Future<void> _sayCannotCome() => leaveEventWithReason(
        context: context,
        service: widget.firestoreService,
  ...
  Future<void> _sayGoing() async {
    ...
    // «НЕ СМОГУ» ИЗ ОКНА КОНФЛИКТА — ТА ЖЕ ДОРОГА, ЧТО У КНОПКИ (17.09).
    //
    // Прежде выбор писал `cant` и на этом кончался. Теперь отказ один и идёт
    // с причиной; ход берётся ТОТ ЖЕ, а не пишется здесь заново — иначе два
    // места решали бы, что значит «не смогу», и разошлись бы молча (N49).
    if (choice == AnswerConflictChoice.cannotGo) {
      await _sayCannotCome();
      return;
    }
    ...
    if (widget.form == _AnswerCardForm.footer) {
      return Column(
        ...
        children: [
          ...
          // ОТКАЗ — ОДНОЙ ДОРОГОЙ, С ПРИЧИНОЙ (17.09).
          //
          // Здесь стояла «Bacarmıram»: она писала `cant`, человек оставался в
          // составе, причины не было. Кнопка не снята, а переименована — то же
          // место, то же действие по смыслу, но ведёт в лист причины.
          //
          // `selected` ЗДЕСЬ БОЛЬШЕ НЕТ, и это не потеря. Прежде она
          // показывала выбранный ответ; выход ответом не остаётся — вышедший
          // видит не эту карточку, а состав с пометкой «İşdən çıxdı».
          _answerButton(
            label: 'Gələ bilmirəm',
            selected: false,
...
class _PersonalEventDetailScreenState
  ...
  // навсегда.
  /// ВЫХОД НАВЕРХ — вернуть вечер в силу из «под вопросом».
  ///
  /// Работа 7, шаг 8, клиентская половина, 09.09.
  ///
  /// **ЭТОТ КОММЕНТАРИЙ БЫЛ ОСИРОТЕВШИМ, и это найдено здесь же 09.09.** Он
  /// начинался словами «ONSUZ DAVAM EDİRƏM — возврат вечера в силу после
  /// ухода участника» и описывал ход, снятый с карточки 12.08; функции под
  /// ним не было, и он приклеился к соседнему `_removeLeftMember`, то есть
  /// **описывал чужой ход**. Тот же класс, что доккомментарий, отрезанный от
  /// своего класса вставкой (пойман в этом же заходе). Ни один сторож проекта
  /// такого не ловит.
  ///
  /// **НАДПИСЬ НЕ ЗАШИТА ЗДЕСЬ, а берётся у `restoreLabel`** — у двух поводов
  /// она разная, и решать это на экране значило бы завести второе место, где
  /// живёт то же решение (N49).
  ///
  ...
  /// даёт обещание составу, и составу же уйдёт уведомление.
  ///
  /// **Отказ не глотается.** Правило `restoresEvent()` пускает только
  ...
  Future<void> _restoreEvent(
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (d) => AlertDialog(
        ...
        content: const Text(
          'Tədbir yenidən qüvvəyə minəcək və heyət bundan xəbər tutacaq.',
        ...
        actions: [
          ...
          TextButton(
            ...
            child: Text(label, style: const TextStyle(color: kGold)),
          ),
        ],
      ),
    );
  ...
  /// УДАЛИТЬ ВЫШЕДШЕГО ИЗ СОСТАВА — крестик в строке участника (29.08).
  ///
  /// **Спрашивает подтверждение, потому что ход необратим для ДРУГОГО
  /// человека** (I29). Состав и право видеть в базе — одно поле `musicians`,
  /// поэтому удалённый теряет мероприятие из виду, и вернуть себя сам он не
  /// сможет: это умеет только владелец. Слова последствия берутся из
  /// `leftMemberActions`, а не пишутся здесь — иначе кнопка и предупреждение
  /// разъедутся при первой правке текста (N49).
  ///
  /// **Пишет `_writeEventEdit`, а не своя запись.** Обвязка заведена 12.08
  /// ровно для этого второго пути правки состава и с тех пор не звалась ни
  /// разу; она несёт три параметра, которые второй вызывающий забывает
  /// (`previousAnswers`, `ownerUid`, `previousParticipants`), и ни один из
  /// них не виден по имени функции.
  ///
  /// **Уведомления удалённому НЕ БУДЕТ** — сервер гасит его, увидев в
  /// `before.answers` подпись самого ушедшего (решение владельца 29.08,
  /// разбор в `eventNotifications.ts`). Гашение живёт на сервере, а не здесь:
  /// клиент не решает, кому писать, и не должен начинать.
  ///
  /// Отказ не глотается: `permission-denied` здесь означает, что состав уже
  /// изменился под руками, и человек обязан узнать об этом словами.
  Future<void> _removeLeftMember(
    PersonalEvent event,
    String memberUid,
    String memberName,
    FirestoreService service,
  ) async {
    // Из двух ходов окошка берётся ИМЕННО удаление, по имени поступка, а не
    // по месту в списке: `.first` завтра окажется другим ходом, и спросят у
    // человека не о том (I11).
    final action = leftMemberActions(name: memberName)
        .firstWhere((a) => a.deed == kLeftActionRemove);
    final ok = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        backgroundColor: kBg2,
        title: Text(action.label, style: const TextStyle(color: kText)),
        content: Text(action.note, style: const TextStyle(color: kMuted)),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            child: Text(action.label, style: const TextStyle(color: kRed)),
          ),
        ],
      ),
    );
    ...
    try {
      await _writeEventEdit(
        ...
        notes: event.notes,
        // Состав без него. Копия, а не правка на месте: список принадлежит
        // модели, и менять его здесь значило бы менять то, что нам показали.
        musicians: [
          for (final u in event.participantUids)
            if (u != memberUid) u,
        ],
      ...
      FirebaseCrashlytics.instance.recordError(
        e,
        st,
        reason: 'agreements_screen: remove left member failed',
      );
    }
  }
  ...
  /// «ÖZ ADAMLARIMI ÇAĞIR» — позвать своих в состав вечера.
  ///
  /// Работа 7, шаг 4 (`docs/plan.md`), 08.09.
  ///
  /// **БЕЗ ЕДИНОГО УСЛОВИЯ ПО `isAgree`, и это решение владельца 08.09 по
  /// первому вопросу работы: кнопка НА ОБОИХ** — и на договоре, и на личном
  /// вечере. Довод его же: «позвать своих» — про вечер, а не про то, платят
  /// ли за него; собирают команду одинаково. Числа за то же: из девяти
  /// составов больше двух человек **все девять** — на личных вечерах, а на
  /// договоре состав собрать нечем вовсе (`musicians` там пишет сервер как
  /// ровно две стороны).
  ///
  /// **Разделение вернуло бы `isAgree` в показ, откуда его убрали шагом 6
  /// 12.08.** Поэтому условия здесь нет и появиться не должно: увидев в этом
  ...
  ///
  /// **ПОРЯДКА ЗАПИСЕЙ БОЛЬШЕ НЕТ — ЗАПИСЬ ОДНА** (17.09, закон о договоре).
  /// Здесь стояло «сперва дети, потом ОДНОЙ операцией шаблон», и довод был
  /// верен: оборванный ход оставлял детей живыми, а они сами себе свидетели.
  /// Детей больше нет — позванный попадает в состав ЭТОГО вечера, — и состав,
  /// ответы и шаблон уходят одной записью. Значит «оборвалось посередине»
  /// невозможно по устройству: либо позвали и записали, либо не случилось
  /// ничего.
  ///
  /// **ЧТО ИЗ ЭТОГО СЛЕДУЕТ ДЛЯ ОТКАЗА, И ЭТО СМЕНА ПОВЕДЕНИЯ, А НЕ УБОРКА.**
  /// Прежде запись шла по одной на человека, поэтому и `kNotInvitedFailed`
  /// был на человека. Теперь запись одна: отказ базы означает «не ушло
  /// НИКОМУ», и окно скажет именно это. Частичность осталась там, где она
  /// содержательна, — открытый раунд, он у каждой пары свой.
  ///
  /// **ЗОВУЩИЙ ТЕПЕРЬ ВИДИТ ПОЗВАННОГО СРАЗУ** — он в составе вечера, и строки
  /// раздела «кого я позвал» читают тот же документ (`lineupRows`). Прежняя
  /// оговорка «самого приглашения не видит нигде, оно схлопнуто под
  /// родителем» снята вместе со схлопыванием.
  Future<void> _callMyPeople(
    ...
    // УЖЕ ПОЗВАННЫЕ — ЭТО СОСТАВ ВЕЧЕРА, и спрашивать больше некого.
    ...
    for (final target in targets) {
      ...
      if (target.outcome == CallOutcome.askAgain) {
        ...
        try {
          ...
          // Здесь отказ — это чтение чата, а не запись вечера: запись одна и
          // идёт ниже.
    ...
    // ОДНА ЗАПИСЬ НА ВЕСЬ ХОД: состав, ответы, шаблон и подпись поступка.
    var slotsAfterWrite = slots;
    try {
      await service.callPeopleToEvent(
        event: event,
        callerUid: myUid,
        added: toAdd,
        recalled: toRecall,
        ...
        // подмены не видит (разбор у `callPeopleToEvent`).
        slots: slots,
      );
    } catch (_) {
      // ОТКАЗ ЕДИНСТВЕННОЙ ЗАПИСИ — ЭТО «НЕ УШЛО НИКОМУ», И СКАЗАТЬ НАДО
      // ИМЕННО ЭТО. Прежде здесь молчали: шаблон был «про следующий раз», а
      // об отправке уже отчитались дети. Теперь отправка И ЕСТЬ эта запись,
      // и промолчать о её отказе значило бы показать окно «Çağırış
        ...
        for (final s in slots)
          !sending.contains(s.uid)
              ...
              : LineupSlot(
                  ...
                  name: s.name,
                  invited: false,
                  reason: s.reason ?? kNotInvitedFailed,
                ),
      ];
    }
  ...
  /// **ТЕЛО ПЕРЕЕХАЛО В `leaveEventWithReason` (17.09), И ВОТ ЗАЧЕМ.** Со
  /// снятием «Bacarmıram» отказ идёт одной дорогой, а дверей в неё стало две:
  /// карточка вечера и экран приглашения. Вторая — `_InvitationScreen`, чужой
  /// класс, и позвать отсюда он ничего не может. Оставь мы тело здесь,
  /// пришлось бы написать его во второй раз — а это N49 дословно: два места,
  /// решающих одно, расходятся молча.
  Future<void> _leaveEvent(
    PersonalEvent event,
    FirestoreService service,
  ) =>
      leaveEventWithReason(
        context: context,
        service: service,
        eventId: event.id,
        uid: widget.currentUid,
              ...
              child: IntrinsicHeight(
                child: Column(
                  ...
                  children: [
                    ...
                    // попадает сюда, а что именно случилось — на экране не
                    // написано нигде. На одной трубке iOS прячет текст
                    // уведомления, и новость теряется совсем; но и с видимым
                    // текстом уведомление читается ОДИН раз — открывший
                    // приложение позже не узнаёт ничего.
                    //
                    // ПРАВИЛО СНАРУЖИ (`eventDeedLine`), а не условие здесь:
                    // значений у поступка пятнадцать, четыре из них обязаны
                    // молчать, и проверить их поимённо можно только тестом.
                    // Та же причина, что у `offerFeedLine` и
                    // `offerAuthorLine`.
                    //
                    // МОЛЧАНИЕ ЗДЕСЬ ОЖИДАЕМО, И ЭТО ЗАПИСАНО ДО РАБОТЫ
                    // (N178): у 58 вечеров из 103 поступка нет вовсе, а
                    ...
                    // ПЛАШКА СОСТОЯНИЯ СНЯТА ЦЕЛИКОМ (решение автора
                    // 13.08) И ОСТАЁТСЯ СНЯТОЙ. Была: показывала «Dəqiq» и
                    // открывала окошко на три пункта — в силе, под вопросом,
                    // отменён.
                    //
                    // Строка выше её НЕ возвращает и не заменяет: та говорила
                    // о СОСТОЯНИИ вечера, эта — о последнем ПОСТУПКЕ. Два
                    // разных вопроса, два правила (I47, N179).
                    //
                    ...
                    // 4. ЛЮДИ НА ВЕЧЕРЕ — ОДИН СПИСОК (решение владельца 17.09,
                    // вариант Б). Компактными строками: кружок с двумя буквами
                    // в обводке по ответу, имя, под ним ответ. Без крупных
                    // карточек и без стрелок: стрелка обещает переход, а тут
                    // переход не главное.
                    //
                    // ЗДЕСЬ БЫЛО ДВА РАЗДЕЛА — «TOY HEYƏTİ» и «ÇAĞIRILANLAR», —
                    // и второй снят 17.09 вместе с доводом, который его держал.
                    // Разделять их было осмысленно, пока позванный жил в СВОЁМ
                    // документе: множества не пересекались по устройству
                    // данных. Отдельные документы сняты — и один и тот же
                    // человек вышел на экран дважды (вечер
                    // `qrqTFjiZ8sP5F09eoXAU`, найдено глазами на трубке).
                    //
                    // КТО В СПИСОК ВХОДИТ И В КАКОМ ПОРЯДКЕ — решает правило
                    // `partyRows`, а не эта разметка: его можно прогнать, а
                    // разметку нельзя (I32). Оно же держит главное
                    // утверждение — ОДИН UID, ОДНА СТРОКА.
                    ...(() {
                      final rows = partyRows(
                        event: event,
                        // Непозванные живут в шаблоне; отбор `invited == false`
                        // делает правило, а не вызывающий.
                        notInvited: event.lineup,
                        // Непозванных видит только владелец — условие внутри
                        // правила, здесь только смотрящий.
                        viewerUid: currentUid,
                      );
                      ...
                      ...(() {
                        ...
                        return rows.map((r) {
                        final uid = r.uid;
                        // ЧЕЛОВЕК В СОСТАВЕ ИЛИ НЕПОЗВАННЫЙ — признак берётся
                        // у правила, а не выводится здесь из пустого ответа.
                        // «Ответа нет» одинаково истинно у того, кого не
                        // спрашивали, и у того, чей документ старый (I47).
                        final isMember = r.kind == PartyRowKind.member;
                        ...
                        // решениям расходиться молча, если между ними
                        // когда-нибудь окажется правка. Сторож по исходникам
                        // считает читателей и краснеет на втором.
                        //
                        // ЧИТАЕТСЯ ИЗ СТРОКИ, А НЕ ИЗ ВЕЧЕРА: у непозванного
                        // `answerFor` дал бы `null` и был бы прав, но спросить
                        // его значило бы завести ВТОРОГО читателя ответа.
                        ...
                        final leaveNote = isMember ? leaveNotes[uid] : null;
                        ...
                        return _PartyMemberRow(
                          ...
                          // РОВНО ЗДЕСЬ и требует, чтобы имя стороны бралось
                          // по uid из списка пользователей: подними вызов
                          // строкой выше — и правило станет невидимым ему,
                          // оставшись верным. Цена — один лишний проход по
                          // списку на строку состава; правило дороже.
                          //
                          // Пустое имя и uid — разное, поэтому запасные
                          // ответы у двух мест РАЗНЫЕ, а не забыты. Показу
                          // uid лучше пустоты: строка без подписи не строка.
                          // Вопросу перед удалением наоборот — там пустое
                          // даёт безличное «İştirakçı», а uid показал бы
                          // человеку машинный код вместо имени.
                          // ЗАПАСНОЕ ИМЯ РАЗНОЕ У ДВУХ ВИДОВ СТРОКИ, И ЭТО НЕ
                          // РАЗНОБОЙ. Участник есть в живом списке людей —
                          // промах здесь означал бы поломку, и `uid` на экране
                          // её показывает. Непозванный мог исчезнуть из списка
                          // СОВСЕМ — ради этого случая в записи и лежит снимок
                          // имени; машинный код ему не подпись, а пустота
                          // честно значит «сведений нет».
                          name: _findUser(allUsers, uid)?.name ??
                              (isMember
                                  ? uid
                                  : (r.snapshotName.isEmpty
                                      ? 'Naməlum'
                                      : r.snapshotName)),
                          photoURL: _findUser(allUsers, uid)?.photoURL,
                          emoji: _findUser(allUsers, uid)?.emoji,
                          answer: answer,
                          // ПРИЧИНА ВМЕСТО ОТВЕТА — у непозванного вопроса не
                          // задавали, и молчать о нём нельзя: человек ждал бы
                          // ответа, которого никто не обещал (решение
                          // владельца 08.09).
                          note: isMember ? null : _notInvitedNote(r.reason),
                          // ДВЕРЬ ТОЛЬКО ТУДА, ГДЕ ЕЙ ЕСТЬ ЧТО ОТКРЫТЬ: закрыть
                          // раунд можно в чате, а у отказа записи чат ни при чём.
                          onOpenChat: !isMember && r.reason == kNotInvitedOpenRound
                              ? () => openDirectChat(
                                    context,
                                    ref,
                                    myUid: currentUid,
                          ...
                          // с 13.08 (N179).
                          //
                          // Условие берётся ЦЕЛИКОМ из правила, а не
                          // складывается здесь из `isOwner` и сравнения
                          // uid: готовые признаки можно вычислить
                          // несогласованно, и разметка подтвердила бы то,
                          // чего нет. Три uid и ответ — всё, что ему нужно.
                          onRemove: offersLeftMemberRemoval(
                            viewerUid: currentUid,
                            memberUid: uid,
                            ownerUid: event.ownerUid,
                            answer: answer,
                          ...
                          //
                          // «?» открывала окошко из двух выборов —
                          // «Dəqiqləşdir» (спросить в чате) и «Siyahıdan sil»
                          // (убрать из состава). Довод автора при снятии:
                          // вышедший помечен «İşdən çıxdı», этого достаточно,
                          // а спросить можно в чате, — то есть окошко
                          // предлагало дорогу, которая и так есть.
                          //
                          // КРЕСТИК НЕ ВОЗВРАЩАЕТ ОКОШКО, ОН ДЕЛАЕТ ОДИН ХОД
                          // ИЗ ДВУХ. Довод владельца 29.08 дословно: «если
                          // Рафаэль вышел из мероприятия Теймура, нет смысла
                          // прятать текст, что Рафаэль вышел, — нужно
                          // удалить, потому что Рафаэль всё равно вышел
                    ...
                    // ЗДЕСЬ БЫЛ РАЗДЕЛ «ÇAĞIRILANLAR» — «кого я позвал», 99
                    // строк, заведённый пробой 09.09 (N210) и снятый решением
                    // владельца 17.09 вместе со вторым списком.
                    //
                    // ПОЧЕМУ СНЯТ, А НЕ ПОПРАВЛЕН. Раздел отвечал на вопрос
                    // «кого я позвал и чем это кончилось», и отвечал верно,
                    // пока позванный жил в ОТДЕЛЬНОМ документе: в состав вечера
                    // он не попадал, и два списка не пересекались. Сняв
                    // отдельные документы, мы сделали позванного участником —
                    // и раздел стал вторым видом на тех же людей. Теймур вышел
                    // на экран дважды, оба раза «gəlir».
                    //
                    // ЧТО ИЗ НЕГО ПЕРЕЖИЛО СНЯТИЕ, названо поимённо, чтобы
                    // следующий не счёл потерянным:
                    //   • «позван и ответил», «позван и молчит» — это строки
                    //     состава, они были там и раньше;
                    //   • «не позван, с причиной» — строка в том же списке,
                    //     с причиной вместо ответа и дверью в чат;
                    //   • «снят» — НЕ СТРОКА. Убрать человека значит убрать его
                    //     из состава, а убранного в вечере нет (решение
                    //     владельца 17.09).
                    ...
                    if (isOwner) ...[
                      ...
                      // «ÖZ ADAMLARIMI ÇAĞIR» — работа 7, шаг 4 (08.09).
                      //
                      // ОДНО МЕСТО ВСТАВКИ НА ВСЁ, и это следствие шага 6
                      // (12.08): карточка теперь одна на договор и на личный
                      // вечер, `_AgreementDetailScreen` снят. До того же
                      // ответа «на обоих» пришлось бы вставлять дважды.
                      //
                      // **УСЛОВИЯ ПО `isAgree` ЗДЕСЬ НЕТ И БЫТЬ НЕ ДОЛЖНО** —
                      // решение владельца 08.09 по первому вопросу работы.
                      //
                      // Условие ниже — про СОСТОЯНИЕ, а не про вид вечера, и
                      // оно то же, что у «Ləğv et»: звать людей в прошедший
                      // или отменённый вечер бессмысленно, а кнопка, которой
                      // некуда вести, читается как поломка (N65, N146).
                      ...
                      // ВЫХОД НАВЕРХ — вернуть вечер в силу. Работа 7, шаг 8,
                      // клиентская половина, 09.09.
                      //
                      // ЗДЕСЬ СТОЯЛО: «„Şübhə altına al“ и „Onsuz davam
                      // edirəm“ СНЯТЫ 12.08: первую заменило окошко выбора на
                      // плашке, вторую — „убрать из состава“ у вышедшего».
                      // Про первую верно и сегодня. Про вторую — **было
                      // верно ровно до появления второго повода**: «убрать из
                      // состава» отвечает на уход участника и не отвечает
                      // ничего на исчезнувшую работу, где участники все на
                      // месте. Записано снятием, а не стёрто (N108).
                      //
                      // УСЛОВИЯ ЗДЕСЬ НЕТ НИ ОДНОГО СВОЕГО. Показывать или
                      // нет — решает `showsRestore`, повторяющий три условия
                      // правила `restoresEvent()`; что написать — решает
                      // `restoreLabel`, у двух поводов надписи разные.
                      // Напиши мы условие тут, оно разошлось бы с сервером
                      // молча, и человек получил бы кнопку, которой отказывают
                      // (I32: порядок ветвей в разметке не сторожит никто).
                      if (showsRestore(
                        isOwner: isOwner,
                        status: event.status,
                      )) ...[
...
/// Шапка блока состава: «TOY HEYƏTİ · 3 NƏFƏR».
///
/// **ВЫШЕДШИЕ В СЧЁТ НЕ ВХОДЯТ — решение владельца 29.08, шаг 2 работы
/// N121.** Довод его: «N NƏFƏR» отвечает на «сколько придёт», а вышедший не
/// придёт. До новой схемы вопрос не стоял: уход вычёркивал человека из
/// `musicians`, и счёт падал сам собой. Теперь человек остаётся в составе —
/// строкой с пометкой «İşdən çıxdı», — и не вычти мы его здесь, число
/// осталось бы прежним при уходе половины состава.
///
/// **«BACARMIR» ИЗ СЧЁТА НЕ ВЫЧИТАЕТСЯ, И ЭТО НЕ ЗАБЫВЧИВОСТЬ.** Он тоже не
/// придёт в этот раз, но из состава не уходил: изменения ему идут, дату
/// подвинут — он может передумать (`recipientsOf` не снимает его никогда, и
/// довод записан там). Строка считает СОСТАВ, а вышедший — единственный, кто
/// из состава выбыл, оставшись в поле `musicians` ради права видеть.
///
/// Разведи эти два случая иначе — и придётся отвечать, что показывать при
/// «ждём ответа» и «не спрашивали»; ответа на это нет, а число одно (I47).
String _partyHeader(PersonalEvent e) {
  final n = e.participantUids
      .where((uid) => e.answerFor(uid) != kAnswerLeft)
      .length;
  final head = e.type.isEmpty ? 'HEYƏT' : '${azUpperCase(e.type)} HEYƏTİ';
  return '$head · $n NƏFƏR';
}

/// ЗДЕСЬ БЫЛИ ТРИ ПОМОЩНИКА РАЗДЕЛА «ÇAĞIRILANLAR» — `_lineupHeader`,
/// `_lineupAnswerOf` и `_lineupNoteOf`. Сняты 17.09 вместе с самим разделом.
///
/// `_lineupHeader` считал «позвано N из M» — теперь считать нечего: позванные
/// и есть состав, и его длину говорит `_partyHeader` выше.
///
/// `_lineupAnswerOf` подставлял непозванному `kAnswerNotAsked` в поле ответа.
/// Снят с доводом: `notAsked` — не ответ, а ВЫВОД из отсутствия ключа, и
/// писать его в строку значило утверждать отсутствие наличием. У непозванного
/// ответа нет вовсе, и в строке на его месте стоит причина.
///
/// Из третьего осталось только то, ради чего он и был нужен, — слово о
/// причине.

/// ОТЧЕГО НЕ ПОЗВАЛИ — словом, которое читает человек.
///
/// **Молчать нельзя, и это решение владельца 08.09, а не оформление:** человек
/// не должен узнавать о непозванном из тишины — иначе будет ждать ответа,
/// который не придёт.
///
/// **НЕЗНАКОМАЯ ПРИЧИНА НЕ ПРЕВРАЩАЕТСЯ В ПУСТОТУ.** `_ => 'səbəb bilinmir'`
/// стоит нарочно: пустая вторая строка прочиталась бы как «всё в порядке», то
/// есть молчание, выданное за успех (I14). Значение приходит из документа, а
/// документ пишет не только наш клиент (I49).
///
/// **`kNotInvitedFailed` ЗДЕСЬ ОСТАЁТСЯ, ХОТЯ ЗАПИСАН БЫТЬ УЖЕ НЕ МОЖЕТ.**
/// После свода писателей в один (`callPeopleToEvent`) запись одна: упади она —
/// упадёт и сама запись о неудаче, вписывать её некуда. То есть значение
/// живёт только на экране, в окне отправки, и в документ не попадает. Слово
/// оставлено ради старых документов и ради того дня, когда путей записи снова
/// станет два.
String _notInvitedNote(String? reason) => switch (reason) {
      kNotInvitedOpenRound => 'onunla artıq iş danışığı gedir',
      kNotInvitedFailed => 'göndərmək alınmadı',
      _ => 'səbəb bilinmir',
    };
...
class _PartyMemberRow extends StatelessWidget {
  ...
  ///
  /// **Показывать его решает не эта строка, а правило
  /// `offersLeftMemberRemoval`** (`core/agreements/left_member_actions.dart`),
  /// и сюда приходит уже готовый ответ: `null` значит «не предлагать».
  /// Условие живёт там, потому что его можно прогнать тестом, а разметку —
  /// нет (I32).
  ///
  /// **Здесь была кнопка «?», снятая 13.08 и мёртвая с тех пор** (N179).
  /// Крестик встал на её место 29.08 по решению владельца и делает не то же
  ...
  Widget build(BuildContext context) {
    final (ring, word) = switch (answer) {
      ...
      // ВЕТВЬ `cant` СНЯТА 17.09 вместе с ответом. Цвета остались: ими
      // красится ВЫШЕДШИЙ (ниже) и строка ухода — их имя переживёт
      // значение, и переименование поедет отдельным шагом, чтобы не
      // мешать снятие с правкой вида.
      ...
      // ВЫШЕДШИЙ ПОЛУЧИЛ ЦВЕТ УХОДА — решение владельца 29.08, вместе с
      // крестиком. До этого он падал в серое умолчание вместе с «не
      // спрашивали», то есть **самое сильное «нет» выглядело тише всех**.
      //
      // ЦВЕТ ВЗЯТ НЕ НОВЫЙ, А ТОТ, КОТОРЫМ УХОД УЖЕ ГОВОРИТСЯ В ЭТОМ
      // ПРИЛОЖЕНИИ: `kAnswerCantText` красит строку «{Ad} ayrıldı» на
      // карточке с 28.08. Заводить «ушёл» свой шестой цвет нельзя — красный
      // на этих экранах уже значит три разные вещи (N110), а палитра ломает
      // собственное правило (N111); обе находки уехали в работу о форме
      // статуса, и красить врозь значит трижды трогать одну палитру (I51).
      //
      // ОТСЮДА СОВПАДЕНИЕ С «BACARMIR», И ОНО СНЯТО НЕ ЦВЕТОМ, А ЗНАЧКОМ.
      // В этом же списке «не может» покрашен так же, и один цвет на двоих
      // слил бы два разных состояния (I47): «не приду в этот раз» и «меня
      // в этом вечере больше нет». Различие несёт `person_remove_outlined`
      // ниже — тот же значок, что у строки ухода на карточке, то есть
      // словарь у них общий, а не выдуманный здесь заново.
      kAnswerLeft => (kAnswerCantBorder, kAnswerCantText),
      // Сюда падают «не спрашивали» и незнакомое. Цвет тот же, каким
    ...
    return Padding(
        ...
        child: Row(
          ...
          children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          // ЗНАЧОК УХОДА — ОН ЖЕ ЕДИНСТВЕННОЕ, ЧЕМ «вышел»
                          // ОТЛИЧАЕТСЯ ОТ «bacarmır» НА ЭТОМ ЭКРАНЕ.
                          // Цвет у них общий по доводу выше, поэтому вся
                          // разница держится на этой строке: снимут значок —
                          // и два разных состояния станут неразличимы, а
                          // покраснеет от этого только сторож в
                          // `source_invariants_test.dart`.
                          //
                          // Тот же `person_remove_outlined`, что у строки
                          // «{Ad} ayrıldı» на карточке: одно событие —
                          // один знак, иначе человек учит два словаря.
                          if (answer == kAnswerLeft) ...[
                            Icon(
                              Icons.person_remove_outlined,
                              size: 14,
                              color: word,
            ...
            // строки, выглядящие одинаково.
            //
            // ЦВЕТ КРЕСТИКА — ТОТ ЖЕ, ЧТО У СЛОВА ПОД ИМЕНЕМ, и это то же
            // требование владельца, что 28.08 у строки «что случилось»: там
            // крестик стоял серым и читался как ничей — пятнышко у края, к
            // которому непонятно, что относится. Общий цвет говорит, ЧТО
            // именно он уберёт.
            //
            ...
            if (onRemove != null)
              GestureDetector(
                ...
                child: Padding(
                  ...
                  child: Icon(Icons.close, size: 18, color: word),
                ),
              ),
          ],
        ),
...
class _EventFormModalState extends State<_EventFormModal> {
  ...
  Future<void> _replaceEvent(PersonalEvent target) async {
    ...
    try {
      ...
      // ему («İştirakçı ayrıldı»). Оно и есть единственное, что этот ход
      // порождает: до починки N47 уходило три, из них два ложных.
      //
      // ЧЕМ СЕРВЕР УЗНАЁТ ОБ УХОДЕ — ПЕРЕПИСАНО 29.08 (N121, шаг 2).
      // Здесь стояло «`planUpdatePushes` по `lastActionType: 'left'`» —
      // верно до сегодня и неверно теперь: клиент этого поля больше не
      // пишет вовсе. Сервер опознаёт уход по ПЕРЕХОДУ ключа
      // `answers.<uid>` в `'left'` (`leftViaAnswers`, выложено 28.08), и
      // имя вышедшего берёт от uid перехода, а не из `lastActionBy` —
      // тот остался бы от прошлого действия и назвал бы постороннего
      // (N177).
  ...
  /// отвечает «что делать с моим временем», а здесь соглашается на то, из
  /// чего сам не выйдет обратно.
  ///
  /// **ПОЧЕМУ НЕ ВЫЙДЕТ — ПРИЧИНА ПЕРЕМЕНИЛАСЬ 29.08, А СЛЕДСТВИЕ НЕТ.**
  /// Здесь стояло «вернуть себя в чужое мероприятие он не сможет, это умеет
  /// только его владелец», и держалось это на том, что уход **вычёркивал
  /// человека из состава**: без состава нет и права видеть. По новой схеме
  /// (N121, шаг 2) он из состава не пропадает, и правило `answersForSelf()`
  /// переход `left → going` **разрешает** — законный второй вход, сервер его
  /// примет. Дороги к нему нет у ПОКАЗА: вечер исчезает из календаря
  /// вышедшего (`showsInCalendarOf`), push по нему больше не приходит
  /// (`recipientsOf` не считает `'left'` живым), значит открыть карточку и
  /// нажать «Gəlirəm» неоткуда.
  ///
  /// **Расхождение правила и показа здесь НЕ дефект, а решение владельца
  /// 29.08:** правило говорит, что сервер ПРИМЕТ, показ — что человеку
  /// ПРЕДЛОЖЕНО. Захотим дать дорогу назад — менять надо показ, а не права.
  ///
...
//     и без них.
//   • ОБОДОК ИСТОРИИ на строке списка — ПОТЕРЯН, назван потерей и ПРИНЯТ
//     ТАКИМ: решение владельца 19.09 — «в окне выбора людей ободок не нужен,
//     оставляем как есть». Снятый диалог рисовал `StatusRing`, общий лист
//     рисует простой кружок с фотографией.
//
//     ЗАПИСАНО РЕШЕНИЕМ, А НЕ УМОЛЧАНИЕМ, и разница существенная: следующий,
//     увидев список без ободка, прочтёт это как недосмотр и вернёт его —
//     заодно отдав одноместному листу «кому предложить работу», где ободка
//     отродясь не было. Долга здесь нет; девять мест, где ободок про
//     историю, работают как прежде.
// ---------------------------------------------------------------------------

/// ОТКАЗ С ПРИЧИНОЙ — ОДНА ДОРОГА НА ВСЕ ДВЕРИ (17.09, решение владельца).
///
/// **ЗДЕСЬ БЫЛО ДВА СПОСОБА СКАЗАТЬ «НЕТ», И ВТОРОЙ СНЯТ.** «Bacarmıram»
/// писала ответ `cant` — человек оставался в составе, причины не было, и
/// владелец видел отказ без единого слова. «Gələ bilmirəm» выводит с
/// причиной. Довод владельца дословно: это дубликат одного и того же
/// действия. Кнопка не исчезла, а переименована: она стоит везде, где стояла
/// «Bacarmıram», и делает то же, что делала «Gələ bilmirəm».
///
/// **ФУНКЦИЯ ВЕРХНЕГО УРОВНЯ, А НЕ МЕТОД ЭКРАНА, ПО НЕОБХОДИМОСТИ.** Дверей
/// стало две: карточка вечера (`_PersonalEventDetailScreenState`) и экран
/// приглашения (`_MyAnswerCard`, живущий и там, и там). Из второго до первого
/// не дотянуться, а писать тело дважды — N49.
///
/// **ПОРЯДОК ВНУТРИ НЕ МЕНЯЛСЯ И МЕНЯТЬСЯ НЕ ДОЛЖЕН:** сперва голос в
/// хранилище, потом выход с признаком. Не загрузился голос — не уходит и
/// выход, и человек узнаёт об этом словами, а не выходит молча без того, что
/// сказал.
///
/// **ПРИЧИНА НЕОБЯЗАТЕЛЬНА И НИЧЕГО НЕ ВЫДУМЫВАЕТ.** Пусто — уходит только
/// выход. Текст отправляется КАК НАПИСАН: приписать к нему название вечера
/// значило бы сказать за человека то, чего он не писал.
///
/// **`context.mounted`, А НЕ `mounted` СОСТОЯНИЯ, и это не мелочь переезда:**
/// у функции своего состояния нет, а лист живёт дольше любого await внутри.
Future<void> leaveEventWithReason({
  ...
  final result = await showModalBottomSheet<LeaveSheetResult>(
    ...
    backgroundColor: Colors.transparent,
    isScrollControlled: true,
    // Закрыть мимо кнопок нельзя: человек уже начал говорить, почему не
    // может, и случайный смах потерял бы сказанное.
    isDismissible: false,
  ...
  try {
    // Голос грузится ПЕРВЫМ: в вечер уходит ПРИЗНАК, что файл уже лежит.
    if (voice != null) {
    ...
    // Выход и причина — ОДНОЙ ПАЧКОЙ: правило `leaveNotes` требует, чтобы
    // после записи ответ был `left`, и порознь оно причину не пропустит.
    ...
    // Временный файл записи не нужен ни при удаче, ни при отказе — N129.
