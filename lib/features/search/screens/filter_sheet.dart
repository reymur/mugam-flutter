...
class SearchFilters {
  ...
  /// ТЕ ЖЕ ПЯТЬ ПОЛЕЙ, НО ПО ЖИВОМУ ЧЕЛОВЕКУ, А НЕ ЗАПРОСОМ В ИНДЕКС.
  ///
  /// **Стоит вплотную к [toAlgoliaFiltersExcluding] нарочно.** Это два способа
  /// применить ОДНО правило, и разойтись они могут молча: добавит кто-нибудь
  /// шестое поле в фильтр — и половина экранов станет отбирать по шести, а
  /// половина по пяти. Рядом это видно глазом; через экран прокрутки — уже
  /// нет (I42: опаснее всего среднее расстояние).
  ///
  /// **Написано не здесь и не сегодня — вынесено.** Тело жило в
  /// `forward_sheet.dart`, в `_chatMatchesFilters`, и делало там ДВА дела:
  /// доставало собеседника из чата и сверяло человека с фильтрами. Общее у
  /// него с листом выбора людей — только второе, оно и вынесено (I58:
  /// сводить по задаче). Первое осталось у `forward_sheet`, где ему и место.
  ///
  /// **`isActuallyOnline`, А НЕ `online`, И НЕ ОКНО ЗАПРОСА.** Здесь данные
  /// живые, значит и правило берётся живое — то же, по которому красится
  /// кружок «в сети». У алголийского близнеца выше стоит `onlineQueryWindow`,
  /// и это не расхождение, а необходимость: индекс отстаёт десятиминутным
  /// шагом переиндексации, и окно запроса обязано быть ШИРЕ шага, иначе
  /// фильтр перестанет находить кого бы то ни было (N57). На живых данных
  /// расширять нечего — там окно 120 с и настоящий ответ.
  ///
  /// **`activityType` здесь ЕСТЬ, а у алголийского близнеца его нет** —
  /// `User.fromAlgoliaHit` это поле не заполняет вовсе, оно там заглушка, и
  /// отбор по инструменту в индексе делает сервер по денормализованному
  /// `activityInstruments`. То есть на живом человеке отбор идёт по
  /// настоящему полю, а не по его следу.
  ///
  /// Пустой фильтр пропускает всех — ни одно условие не сработает. Вызывающий
  /// всё равно спрашивает [isEmpty] первым: при пустом фильтре список не надо
  /// даже обходить.
  bool matchesPerson(User u) {
    if (city != null && u.city != city) return false;
    if (minRating > 0 && u.rating < minRating) return false;
    if (onlyAvailable && !u.available) return false;
    if (onlyOnline && !u.isActuallyOnline) return false;
    final wanted = activityType?.toSearchableInstruments() ?? const [];
    if (wanted.isNotEmpty) {
      // «Хоть один общий инструмент», а не «все» — ровно те же условия, что у
      // ИЛИ-группы в алголийском близнеце выше.
      final have = u.activityType?.toSearchableInstruments() ?? const [];
