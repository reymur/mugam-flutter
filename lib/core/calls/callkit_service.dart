...
class CallKitService {
  ...
  //
  // ПОМЕТКА СПРАШИВАЕТСЯ, НО НЕ СНИМАЕТСЯ ЧТЕНИЕМ — И ЭТО ПОЧИНКА ПО ЗАМЕРУ,
  // А НЕ ПЕРЕСТРАХОВКА (N195, 06.09).
  //
  // ЗДЕСЬ СТОЯЛО: «Пометка снимается тем же чтением, которым проверяется:
  // второго такого события по одному звонку не бывает». **БЫВАЕТ.** На одно
  // наше смыкание окна плагин шлёт ДВА события подряд, и печать с трубки
  // показала это дословно:
  //
  //   04:41:55.747  endCall(…)  callId=vRDhXUy0mwIgd3g0C6eJ
  //   04:41:55.750  ENDED    … closedByUs=true   ← пометка съедена здесь
  //   04:41:55.754  DECLINE  … closedByUs=false  ← её уже нет, пишется отказ
  //
  // Разрыв 3–6 мс, воспроизведено на двух звонках из трёх. Прежний довод
  // ничем не был замерен и стоял в файле утверждением — а три вердикта
  // сторожа требовали его соблюдения, то есть сторож ЗАКРЕПИЛ догадку. Это
  // отдельная находка, N202.
  //
  // ЧТО СНИМАЕТ ПОМЕТКУ ТЕПЕРЬ: `endCall` ниже, вместе с `callkitId`,
  // наблюдателем и обоими наборами учёта — один раз на звонок, до того как
  // события придут.
  //
  // ЧЕГО ЭТО НЕ РЕШАЕТ, СКАЗАНО ЗДЕСЬ: набор не подрезается, и по звонку в
  // нём остаётся короткая строка до конца жизни процесса. Тем же свойством
  // обладает `_applied` выше, и по той же причине — второго законного
  // события по завершённому звонку не приходит, а перепутать `callId` между
  // звонками нельзя, они уникальны.
  //
    ...
    FlutterCallkitIncoming.onEvent.listen((event) async {
      ...
      switch (event) {
        ...
        case CallEventActionCallDecline(:final callKitParams):
          ...
          if (_closedByUs.contains(callId)) {
            _callkitIdByCallId.remove(callId);
            _endWatchers.remove(callId)?.cancel();
            break;
          }
        ...
        case CallEventActionCallEnded(:final callKitParams):
          ...
          // ВТОРАЯ ПОЛОВИНА N198, НАЙДЕНА ПРОБОЙ НА ТРУБКАХ 06.09 МЕСТНОГО
          // (05.09 UTC). ПОЧИНКА `b18881a` ЗАКРЫЛА ТОЛЬКО «ОКНО ЗАКРЫЛИ МЫ»;
          // ОТКАЗ, НАЖАТЫЙ ЧЕЛОВЕКОМ В СИСТЕМНОМ ОКНЕ CallKit, СЮДА ЖЕ
          // ПРИХОДИЛ И ЗАПИСЫВАЛСЯ КАК ЗАВЕРШЕНИЕ.
          //
          // КОНТРОЛЬНАЯ ПАРА, ОДНА ТРУБКА И ОДНА КНОПКА: `XeUgVKKuh8c3ZsGSNU62`
          // (`23:00:14Z`) — своего исходящего у отклонявшего ещё не было,
          // `respondToCall` вызван, статус `declined`; `J69ROFh1Ynk2VokMxoaJ`
          // (`23:07:54Z`) — тот же человек, та же кнопка, но за 30 с до этого
          // он позвонил сам: `respondToCall` НЕ вызван ни разу, статус `ended`.
          // Различие между ними ровно одно — его собственный исходящий.
          //
          // ПОЧЕМУ ОТЛИЧИТЬ МОЖНО У НАС, ХОТЯ ПЛАГИН НЕ МОЖЕТ. Ему видно
          // только `CXEndCallAction`, одинаковый у всех. Нам видно ТРИ вещи,
          // и все три уже считаются здесь для других надобностей:
          //   • `_closedByUs` — окно закрыли мы (спрошено выше);
          //   • `_outgoingCallIds` — этот звонок наш исходящий, значит мы
          //     звонящий, а не вызываемый, и отказываться нам не от чего;
          //   • `_acceptedCallIds` — разговор начинался.
          // Нет ни одного из трёх — человек положил трубку неотвеченному
          // входящему, а это и есть отказ.
          //
          // ГРАНИЦА, РАДИ КОТОРОЙ ЗДЕСЬ И СТОИТ `_acceptedCallIds`, И ОНА ПОД
          // ВЕРДИКТОМ, А НЕ ПОД ЧЕСТНЫМ СЛОВОМ: отбой из системного окна УЖЕ
          // ИДУЩЕГО разговора — это завершение, и записывать его отказом
          // нельзя. Сними условие — и починка превратит каждое завершение с
          // системного экрана в отказ, то есть заведёт зеркальную ложь вместо
          // прежней.
          //
          // ЧЕГО ЭТА ПРАВКА НЕ ЗАКРЫВАЕТ, СКАЗАНО ЗДЕСЬ, А НЕ ВЫЯСНИТСЯ ПОТОМ:
          //   • отложенное действие `end` из `drainPendingNativeActions` идёт
          //     мимо этой развилки. Оно делит ветвь с `timeout`, а истёкший
          //     звонок отказом НЕ является, и разделять их наугад нельзя —
          //     замера на такой случай нет ни одного (I50). В свежем процессе
          //     защёлка снята, поэтому отказ при мёртвом приложении копится
          //     как `decline`, а не как `end`, — то есть случай возможен, но
          //     не наблюдался;
          //   • три набора живут в памяти процесса. Придёт событие после
          //     перезапуска — они пусты, и завершение отвеченного звонка
          //     прочтётся как отказ. Этим же свойством уже пользуется
          //     `_closedByUs`, и лечится оно не здесь.
          final weAreCallee = !_outgoingCallIds.contains(callId);
          final neverAnswered = !_acceptedCallIds.contains(callId);
          try {
            if (weAreCallee && neverAnswered) {
              await firestoreService.respondToCall(
                callId: callId,
                accept: false,
              );
            } else {
  ...
  Future<void> endCall(String callId) async {
    ...
    // ПОМЕТКА СНИМАЕТСЯ ЗДЕСЬ, ВМЕСТЕ С ОСТАЛЬНЫМ УЧЁТОМ ПО ЗВОНКУ, И ТОЛЬКО
    // ЗДЕСЬ (N195, 06.09). Прежде её снимало ЧТЕНИЕ в обработчике событий, и
    // это было неверно: на одно наше смыкание окна плагин шлёт ДВА события
    // подряд — `ENDED`, через 3–6 мс `DECLINE`. Первое съедало пометку,
    // второму она не доставалась, и второе писало отказ от имени человека.
    // Замер и печать — в записи N195.
    //
    // Снятие стоит ПЕРЕД повторной постановкой ниже нарочно: если по этому
    // же `callId` смыкание случится второй раз, пометка должна взвестись
