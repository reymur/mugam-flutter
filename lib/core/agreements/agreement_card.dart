...
enum AgreementOutcome {
  ...
  ///
  /// **I28 ДЕЙСТВУЕТ ЗДЕСЬ ДО ОПРЕДЕЛЁННОЙ РАБОТЫ, А НЕ НАВСЕГДА.** 10.08
  /// автор утвердил показ статуса в календаре ФОРМОЙ: залито — точно,
  /// рамка — под вопросом, цвет остаётся цветом человека. Это прямо
  /// отменяет довод выше: в календаре появится вопрос, на который у двух
  /// состояний ответы разные, — «на это уже можно рассчитывать?». Пока та
  /// работа не написана, верен абзац выше, и одинаковый вид в календаре —
  /// ВЕРНОЕ поведение, а не пропущенное.
  ///
  /// Записано в коде, а не только в плане, потому что читателя у этого
  /// довода два: правящий календарь придёт сюда за ответом «почему там
  /// пометки нет» — и без этих строк унесёт «так решено» вместо «так решено
  /// до работы X». Разбор и цена — `docs/plan.md`, раздел про статус
  /// формой; шаги проверки — `docs/check-unsettled-status.md`, §5.
  unsettled,
