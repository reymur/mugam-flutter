...
/// (N232, шаг 2).
///
/// **Выводится из тех же двух значений, что и адрес в хранилище**, поэтому в
/// документе вечера хранить путь не нужно вовсе: `eventId` и `uid` и так
/// известны показу. Ради этого шаг 2 обошёлся без нового поля.
///
...
///
/// **ЗДЕСЬ ЖИВЁТ N237, И ССЫЛКА СТОИТ В ОБЕ СТОРОНЫ (I42).** Причины
/// **пересобираются через `toMap()`**, а не переносятся как есть. Пока форма
/// причины одна, это безразлично; с приходом `hasVoice` (N236, шаг 2) —
/// нет: сборка, чей `fromMap` новой формы не знает, прочтёт такую причину
/// как пустую, `toMap()` не запишет ничего, и **правка состава молча сотрёт
/// причину, которой не касалась**. Правилом это не ловится — правка состава
/// идёт владельческой ветвью `allow update`, у которой списка ключей нет
/// вовсе, а законное удаление причины (у убранного крестиком) от потери
/// отличает только замысел. Встречная пометка — в `firestore.rules`, у
/// `leavesWithNote()`.
