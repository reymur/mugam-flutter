...
enum DeedTone {
  ...
  /// Работы не стало. Предупреждающее, СВОИМ тоном (N222, 11.09).
  ///
  /// **ТРЕТИЙ ТОН, А НЕ ПЕРЕКРАСКА ВТОРОГО, и довод у соседа выше цел.** Он
  /// запрещал красить отмену вечера в цвет ухода человека — «тон перестанет
  /// что-либо означать», — и запрет остаётся в силе: `memberGone` по-прежнему
  /// только про людей. Здесь заведён отдельный цвет для отдельной новости.
  ///
  /// **ПОЧЕМУ СЕРОГО НЕ ХВАТИЛО — довод соседа выше верен для СВОЕГО случая и
  /// не переносится на этот.** Он писался про отмену СВОЕГО вечера его же
  /// владельцем: тому новостью это не является, он сам нажал. Здесь другое —
  /// **чужая работа исчезла из-под уже согласившегося человека**, и ему это
  /// новость, требующая решения: держать ли день.
  ///
  /// **И ТОТ «СВОЙ ПОКАЗ», НА КОТОРЫЙ ССЫЛАЛСЯ ДОВОД, СНЯТ С ЭКРАНА 13.08**
  /// (N179, плашка состояния). Тон отдали другому показу, а показ убрали —
  /// осталось серое, читаемое как обычная подпись. Найдено 11.09 владельцем
  /// на снимке карточки.
  workGone,
}
...
///
/// **ПОВТОР ТОГО ЖЕ ПОСТУПКА РАЗЛИЧАЕТСЯ ВРЕМЕНЕМ — четвёртая часть ключа
/// (N181, 01.09).** Ушёл, вернулся, ушёл снова: вечер, имя поступка и автор
/// у обоих уходов одни и те же, и до этой части ключ совпадал — вторая
/// новость оставалась спрятанной первым нажатием, навсегда.
///
/// **Время берётся из документа и прибито правилом:** `lastActionAt ==
/// request.time` (`stampsTime` в `firestore.rules`). Своё время участнику не
/// доверено намеренно — оно ЧАСТЬ ЭТОГО КЛЮЧА, и подделавший попадал бы в
/// чужой прежний ключ: спрятал бы строку, которую человек не прятал.
///
/// **`null` — законный случай, а не поломка:** вечер, тронутый сборкой
/// старше 01.09, отметки не имеет. Такие дают ключ с пустой четвёртой
/// частью, то есть прежней формы по смыслу, но НЕ равный прежнему ключу —
/// разделитель в нём есть. **Старые скрытия отклеятся один раз, и это
/// решение владельца 01.09, а не недосмотр:** читать отсутствие как прежнюю
/// форму значило бы завести второй способ понимать одни данные и держать его
/// в коде навсегда ради одних суток. Отклеившееся чинится нажатием.
///
/// **ЧЕГО КЛЮЧ ПО-ПРЕЖНЕМУ НЕ РАЗЛИЧАЕТ:** два поступка внутри одной
/// миллисекунды. Случай теоретический — поступки делают люди руками, — но
/// назван, чтобы «различает повтор» не читалось шире, чем есть. Настоящее
/// лечение всего класса — лента событий вечера (N178), и она эту работу
/// переживёт: здесь чинится ключ, а не история.
String deedDismissKey({
  ...
  required DateTime? lastActionAt,
    ...
    '|${lastActionAt?.microsecondsSinceEpoch ?? ''}';
...
EventDeed? eventDeedLine({
  ...
  switch (lastActionType) {
    ...
    case kDeedWorkCancelled:
      // ПИСАТЕЛЬ ПОЯВИЛСЯ 11.09 — `unsettleChildrenOnParentCancel`, шаг 8
      // (`docs/plan.md`). Здесь стояло «писателя пока нет — придёт в Части 6»,
      // и это было верно до выкладки 11.09, 23:50:14 UTC.
      //
      // ТОН ПОМЕНЯН ТОГДА ЖЕ, С `plain` НА `workGone` (N222): серым эта
      // строка читалась как обычная подпись, а она сообщает о потере работы.
      return const EventDeed('İş ləğv olundu', DeedTone.workGone);
  }
