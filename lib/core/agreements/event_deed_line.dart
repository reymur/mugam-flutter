...
enum DeedTone {
  ...
  // ЗДЕСЬ СТОЯЛ ТРЕТИЙ ТОН, `workGone` (N222, 11.09 днём), — снят решением
  // владельца 11.09 вечером. Довод: **красный должен значить одно**, и значит
  // он теперь СОСТОЯНИЕ приглашения — «Şübhə altında» (`invitationStateLabel`),
  // строкой выше поступка. Поступок под ним — подпись, «кто и что», и он уже
  // сказан словом; две красные строки подряд перестают выделять обе.
}
...
EventDeed? eventDeedLine({
  ...
  switch (lastActionType) {
    ...
    case kDeedWorkCancelled:
      // ПИСАТЕЛЯ СНОВА НЕТ С 12.09: `unsettleChildrenOnParentCancel` снят
      // решением владельца — отмена одна для всех, без автоматики с детьми.
      // Строка остаётся: имя лежит в документах прода (писатель жил с
      // 11.09, 23:50:14 UTC), и их обязаны прочитать.
      //
      // ТОН СЕРЫЙ, И ЭТО РЕШЕНИЕ, А НЕ ВОЗВРАТ К ПРОПУСКУ. Днём 11.09 здесь
      // стоял предупреждающий (N222); вечером того же дня владелец отдал
      // красный СОСТОЯНИЮ — строке «Şübhə altında» над этой. Поступок — подпись.
