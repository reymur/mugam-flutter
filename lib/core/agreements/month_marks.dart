...
DayMark? dayMarkOf(
  ...
  // СХЛОПЫВАНИЕ ПРИГЛАШЕНИЙ — ПЕРВЫМ ДЕЛОМ, ДО ВСЕХ ТРЁХ ВОПРОСОВ НИЖЕ
  // (N211, 09.09). Пометка, число непросмотренных и след разобранного
  // считаются по одному и тому же перечню, и подмешайся туда дети — каждый
  // из трёх ответов посчитал бы один вечер несколько раз.
  //
  // ЗДЕСЬ ЭТО СЕГОДНЯ НИЧЕГО НЕ МЕНЯЕТ НА ЭКРАНЕ, и сказано это прямо, чтобы
  // строку не сочли пустой: у ребёнка тот же владелец и та же дата, что у
  // родителя, поэтому три буквы и цвет выходят те же. **Совпадение, а не
  // замысел.** Разойдутся они на шаге 8 работы 7: сервер поставит детей под
  // вопрос при отмене родителя, и `anyUnsettled` окрасит день зовущего
  // рамкой «под вопросом» — при том что его собственный вечер под вопросом
  // не стоит.
  //
  // ЦЕНА, НАЗВАННАЯ ДО ПРАВКИ: если родителя удалить, а дети останутся,
  // клетка у зовущего опустеет. Это верно и согласовано с соседями —
  // `showsInCalendarOf` прячет тех же детей из списка дня, — то есть день
  // будет пуст и в клетке, и под ней, а не пуст в одном месте и занят в
  // другом.
  final visible = [
    for (final e in dayEvents)
      if (!collapsesUnderParent(e, currentUid)) e,
  ];
  if (visible.isEmpty) return null;
  final sorted = [...visible]..sort((a, b) => a.date.compareTo(b.date));
