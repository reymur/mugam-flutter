...
/// **[viewerUid] — ЧЕЙ ЭТО ДЕНЬ.** Добавлен 29.08 шагом 2 работы N121.
///
/// **Это не новая служба у функции, а названная вслух старая.** Оба списка,
/// `own` и `asParticipant`, приходят из запросов ЭТОГО человека и всегда
/// принадлежали ему; функция просто не знала, кому именно, потому что до сих
/// пор ни одно её решение от этого не зависело. Теперь зависит одно: вечер,
/// из которого человек ВЫШЕЛ, приходит по-прежнему — по новой схеме он
/// остаётся в `musicians`, — и показывать его нельзя (`showsInCalendarOf`,
/// разбор там же).
///
/// **Почему фильтр здесь, а не в [liveEvents].** `liveEvents` отвечает «живо
/// ли мероприятие» — ответ один на всех, и его зовут ещё двое: занятость
/// (`occupied_days.dart`) и сетка месяца через `eventsByDay`. Обоим фильтр
/// не нужен и вреден по разным причинам: занятость сама берёт только
/// `occupied`, а сетка — `dayMarkOf`, и вышедший у обоих уже отсеян ролью.
/// Отдай мы `uid` в `liveEvents`, вопрос «живо ли оно» стал бы вопросом «живо
/// ли оно для меня» у всех троих сразу — то есть одна функция на два разных
/// дела (I58), ради одного зовущего.
///
/// **Пустой [viewerUid] не прячет ничего, и это не умолчание, а свойство
/// правила:** `dayRoleOf` при пустом uid отвечает `occupied` до чтения
/// ответа, значит `showsInCalendarOf` всегда истинно. Сторона выбрана та же,
/// что у `dayRoleOf`: неизвестный смотрящий видит ЛИШНЕЕ, а не теряет своё —
...
DayBuckets buildDayBuckets({
  ...
  final byId = {
    for (final e in liveEvents(own: own, asParticipant: asParticipant))
      if (showsInCalendarOf(e, viewerUid)) e.id: e,
  };
