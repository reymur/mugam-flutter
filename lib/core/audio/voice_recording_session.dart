...
//
// Показа здесь тоже нет — ни `setState`, ни таймера подписи, ни пульсации:
// они у каждого места свои. Мгновенный отклик на нажатие сохранён через
// `onArmed` (см. `start`), потому что порядок «разрешение → показ →
// отложенный настоящий старт» был выверен на телефоне и его нельзя менять
// заодно с выносом.
