...
class User {
  ...
  //
  // Почему это важнее экономии: в шапке чата стоит зелёный кружок, а
  // прямо под ним, в плашке переговоров, — строка присутствия. При разных
  // окнах через минуту после ухода собеседника они начинали
  // противоречить друг другу на одном экране (кружок зелёный и «1 dəq
  // əvvəl» одновременно). Два индикатора, спорящих между собой, хуже
  // одного неточного: человек перестаёт верить обоим.
  //
  // До 18.09 в шапке стояло СЛОВО «● Onlayn», а не кружок, и красилось оно
  // своим зелёным — `0xFF4CAF50` против `kGreen`. Слово снято решением
  // владельца: «в сети» показывается одним и тем же кружком во всех
  // тринадцати местах.
  //
...
class PersonalEvent {
  ...
  /// КОГДА поступок случился — отметка СЕРВЕРА (N181).
  ///
  /// Третья часть отметки о поступке, рядом с «кто» и «что». Заведена, чтобы
  /// два ОДИНАКОВЫХ поступка перестали быть неотличимыми: без неё человек,
  /// спрятавший крестиком «Rafael ayrıldı», не увидел бы и второго такого же
  /// ухода — ключ скрытия у них совпадал (`deedDismissKey`).
  ///
  /// **Тип — сырой, как у соседей `cancelledAt` и `createdAt`.** Приведения
  /// к `Timestamp` здесь нет намеренно: у документа вечера писателей
  /// несколько, и жёсткое приведение падало бы на всяком значении, которое
  /// положил не наш клиент (I49).
  ///
  /// **`null` значит «поступка без отметки» — то есть вечер, тронутый
  /// сборкой старше 01.09, либо не тронутый вовсе.** Это законный случай, а
  /// не поломка, и переходное окно решено ПЕРЕЖИТЬ, а не обходить
  /// (решение владельца 01.09): читать отсутствие как прежнюю форму значило
  /// бы завести второй способ понимать одни данные и держать его в коде
  /// навсегда ради одних суток. Отклеившиеся скрытия чинятся нажатием.
  final dynamic lastActionAt;

  /// То же время, разобранное для тех, кому нужен момент, а не сырьё.
  ///
  /// **Разбор живёт здесь, а не у потребителя, по причине, а не для
  /// красоты:** ключ скрытия строится в `event_deed_line.dart` — чистом
  /// правиле, вынесенном затем, чтобы проверяться БЕЗ Firestore. Затащив
  /// туда `Timestamp`, мы затащили бы туда и зависимость от базы.
  ///
  /// **Здесь два незнания слиты в одно, и это законно ровно потому, что
  /// решается «что показать», а не «норма или поломка» (I47):** отметки нет
  /// (старый вечер) и в поле лежит не отметка (чужой писатель, правка
  /// руками в консоли) — для ключа неразличимы, различать их нечем и
  /// незачем. Сырое поле рядом остаётся: у кого появится вопрос «норма или
  /// поломка», тот прочтёт его.
  DateTime? get lastActionMoment {
    final v = lastActionAt;
  ...
  /// состава (ушедший свой ключ оставлял — `leavesEvent()` трогать `answers`
  /// не давало; правило снято 30.08, ключи остались), а сырая карта не знает
  /// ни того, ни другого.
  ///
  ...
  /// больше (шаг 4).
  ///
  /// Правка состава обязана перенести уже данные ответы, иначе первая же
  /// поправка места объявит ответивших заново ждущими: ответ принадлежит
  /// человеку, а не документу. Чтобы перенести, писателю нужна карта **как
  /// есть**, без разбора — разбирать её будет `answersForParticipants`.
  ///
  /// **Читать через это ответ человека НЕЛЬЗЯ.** Для чтения есть [answerFor]:
  /// он знает про запасной путь у 75 документов без карты, про ключи вне
  /// состава и про то, что отсутствие ключа — «не спрашивали», а не «ждём».
  /// Сырая карта не знает ничего из этого.
  ///
  /// **Имя выбрано так, чтобы читатель не польстился:** не `answers`, а
  /// «карта для перезаписи». Плюс сторож по исходникам
  /// (`test/source_invariants_test.dart`) держит **ровно одно место вызова** —
  /// иначе приватность поля защищала бы форму, а не смысл.
  Map<String, dynamic>? answersForRewrite() => _answers;
  ...
  /// ОТЧЕГО ВЕЧЕР СТОИТ «ПОД ВОПРОСОМ» — своё поле с 08.09 (работа 7, шаг 1).
  ///
  /// **ПИШЕТ ТОЛЬКО СЕРВЕР.** Клиенту оно запрещено правилом
  /// `serverOwnsUnsettledReason()` — целиком, включая владельца: по нему
  /// `restoresEvent()` решает, открывать ли выход наверх, и отдай мы его
  /// участнику, он открыл бы себе выход сам.
  ///
  /// **ПОЛЕ ПИСАЛОСЬ С 08.09, А ЧИТАТЬ ЕГО КЛИЕНТ НАЧАЛ ТОЛЬКО 09.09 — и это
  /// находка, а не порядок работ.** Правило `showsRestore` завели 08.09 вместе
  /// с полем, оно принимает повод параметром, и подать его было НЕЧЕМ: модель
  /// поля не знала. То есть правило было недостижимо с рождения, а выглядело
  /// готовым — у него и тест был зелёный, потому что тест подаёт повод сам.
  /// Ровно тот класс, что I9: проверка, которая не может провалиться, ничего
  /// не доказывает про прод.
  ///
  /// **`null` — «повода нет», и это НЕ то же, что «повод неизвестен».**
  /// Замер 07.09: поля нет у 121 документа прода из 121. Значит `null` здесь
  /// сегодня — обычная жизнь, а не признак поломки, и `showsRestore` на нём
  /// честно молчит: выход наверх открывать не по чему.
  final String? unsettledReason;

  /// ЧЬЁ ЭТО ПРИГЛАШЕНИЕ — id вечера, ради которого человека позвали.
  ///
  /// Работа 7, шаг 5 (`docs/plan.md`). Устройство выбрано 09.08 и не
  /// пересматривалось: каждое приглашение — **отдельный документ**, а не
  /// подколлекция и не поле состава на родителе. Довод владельца: этот вариант
  /// не трогает `firestore.rules` и не трогает сторону приглашённого, которая
  /// уже правильная — у него это одна работа, своя карточка, свой ответ.
  ///
  /// **`null` у подавляющего большинства и будет им всегда** — обычный вечер
  /// ничьим приглашением не является. Это не «сведений нет», а «вечер сам по
  /// себе»; читателю различать тут нечего.
  ///
  /// **Что из этого поля следует прямо сейчас, а не когда-нибудь:** у ребёнка
  /// `ownerUid` — ЗОВУЩИЙ, значит ребёнок приходит зовущему тем же запросом,
  /// что и его собственные вечера, и без правила схлопывания стоял бы у него в
  /// календаре отдельной строкой. Пять позванных дали бы пять строк на 20:00 —
  /// это не пять дел, а одно, показанное пять раз. Правило —
  /// `collapsesUnderParent` в `core/agreements/day_role.dart`.
  final String? parentEventId;

  /// КОГО ЗВАЛИ НА ЭТОТ ВЕЧЕР — шаблон состава и отметка непозванного.
  ///
  /// Живёт на РОДИТЕЛЕ. Разбор формы и трёх состояний записи —
  /// `core/agreements/lineup.dart`; здесь только то, что нужно читателю
  /// модели.
  ///
  /// **Пустой список у всех, кроме родителей приглашений.** Отсутствие поля и
  /// пустой список сведены в одно намеренно, и это законно ровно потому, что
  /// решается «что показать», а не «норма или поломка» (I47): звать не начали
  /// и позвали никого — для показа неразличимы и обязаны быть неразличимы.
  ...
  factory PersonalEvent.fromFirestore(String id, Map<String, dynamic> data) {
    return PersonalEvent(
      ...
      // ЧТЕНИЕ ЗАЩИТНОЕ, и жёсткое приведение здесь стояло бы зря — тот же
      // довод, что у карты ответов ниже (I49). Поле новое, писатель у него
      // сегодня один — наш клиент, — но документ вечера правят ещё сервер
      // мимо правил и рука в консоли, а падение здесь роняет не поле, а весь
      // календарь: `fromFirestore` зовётся на каждый документ каждого потока.
      // Чтение защитное по той же причине, что у соседей (I49): поле пишет
      // сервер мимо правил, и чужой тип уронил бы весь календарь.
      unsettledReason: data['unsettledReason'] is String
          ? data['unsettledReason'] as String
          : null,
      parentEventId:
          data['parentEventId'] is String ? data['parentEventId'] as String : null,
      lineup: lineupFromFirestore(data['lineup']),
