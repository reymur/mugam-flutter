...
class FirestoreService {
    ...
    await FirebaseStorage.instance
        ...
        .putFile(
          ...
          SettableMetadata(
            ...
            // ЗДЕСЬ СТОЯЛ `'firebaseStorageDownloadTokens': ''` — ПРОБА N241,
            // СНЯТА 16.09 ТЕМ ЖЕ ВЕЧЕРОМ: СЕРВЕР ОТКЛОНИЛ ЗАГРУЗКУ ЦЕЛИКОМ.
            //
            // Проверялось, можно ли не снимать токен, а не дать его выдать.
            // Замер на трубке: загрузка падает с `firebase_storage/unknown`,
            // «Unknown 400 error from backend», и выход с причиной голосом не
            // проходит ВООБСЕ — не «файл лёг без токена», а «файл не лёг».
            //
            // ЧТО МЫ СПУТАЛИ, И ЭТО ГЛАВНОЕ: `storage.rules` этот ключ правда
            // не ограничивает — и это ничего не значило. Правила и приём
            // загрузки — РАЗНЫЕ СЛОИ: правило может пустить то, что сервер
            // хранилища не примет. Разрешение в правилах читалось как
            // разрешение вообще.
            //
            // Чинится это триггером на загрузку — разбор в N241.
  ...
  /// ПРОСМОТРЕННЫЕ ПРИГЛАШЕНИЯ — `users/{uid}/seenInvitations/{eventId}`.
  ///
  /// Заведено 12.08 для числа непросмотренных на клетке календаря. Отдаётся
  /// **множеством id**, а не списком документов: у ответа нет ни порядка, ни
  /// повторов, и список позволил бы читателю придумать себе и то и другое.
  ///
  /// **ПУСТОТА ИЗ КЭША ЗДЕСЬ ОПАСНЕЕ, ЧЕМ ГДЕ-ЛИБО, и потому отсеивается
  /// тем же условием, что у `watchStarredMessages` (N14).** Пустой набор
  /// «просмотренного» означает «человек не видел НИ ОДНОГО приглашения» — то
  /// есть противоположность правде для того, кто их все разобрал. Отдав такой
  /// снимок, экран показал бы полное число на дне, где ждать нечего.
  ///
  /// **А пока набор неизвестен, число показывается ПОЛНОЕ, и это выбранная
  /// сторона ошибки.** Ошибиться можно в две: показать лишнее (позвать
  /// смотреть туда, где всё разобрано) или спрятать (промолчать о живом
  /// приглашении). Первое человек закроет одним касанием; второе он не
  /// заметит вовсе — а за ним стоит ждущий ответа человек. **Ложная тревога
  /// дешевле пропуска** (I14), и здесь это не общее рассуждение, а выбор
  /// поведения при незнании.
  Stream<Set<String>> watchSeenInvitations(String uid) {
    return _db
        .collection('users')
        .doc(uid)
        .collection('seenInvitations')
        .snapshots()
        .where((snap) => !(snap.docs.isEmpty && snap.metadata.isFromCache))
        .map((snap) => snap.docs.map((d) => d.id).toSet());
  }

  /// «Я это приглашение видел». Идемпотентно: повторный просмотр ничего не
  /// меняет, поэтому `set` с перезаписью, а не проверка-и-запись.
  ///
  /// **Отметка о времени пишется, хотя её никто не читает.** Это не задел на
  /// будущее, а единственный способ отличить «пометка есть» от «документ
  /// создан пустым по ошибке»: у документа без полей нет ни одного признака,
  /// по которому видно, что его писали намеренно.
  Future<void> markInvitationSeen(String uid, String eventId) {
    return _db
        .collection('users')
        .doc(uid)
        .collection('seenInvitations')
  ...
  Future<String> addPersonalEvent({
    ...
    final ref = await _db.collection('personalEvents').add({
      ...
      // `previousParticipants: null` — У СОЗДАНИЯ ПРЕЖНЕГО СОСТАВА НЕТ
      // ВОВСЕ, и это «не знаем», а не «был пуст» (N173). Разница тут ни на
      // что не влияет — переносить всё равно нечего, `previous` пуст, — но
      // параметр обязателен нарочно: молчаливое умолчание и было той самой
      // дырой, из-за которой согласие воскресало при повторном приглашении.
      'answers': answersForParticipants(participantUids,
          ownerUid: ownerUid, previousParticipants: null),
  ...
  /// ПОЗВАТЬ СВОИХ В ВЕЧЕР — ОДНОЙ ЗАПИСЬЮ В САМ ВЕЧЕР.
  ///
  /// Работа «приглашение живёт в родителе», шаг 1 (17.09). **Заменяет собой
  /// двух прежних писателей — `createLineupInvitation` и `saveLineup`.**
  ///
  /// **ЗДЕСЬ СТОЯЛО ДВА МЕТОДА, И ИХ БЫЛО ДВА НЕ ПО НЕБРЕЖНОСТИ.** Прежнее
  /// устройство заводило на каждого позванного **отдельный документ** с
  /// `parentEventId`, а список отмеченных писало шаблоном на родителя. Отсюда
  /// и два писателя, и правило «сперва дети, потом шаблон»: дети были сами
  /// себе свидетели, и оборванный ход оставлял их живыми.
  ///
  /// **ЗАКОН О ДОГОВОРЕ (`CLAUDE.md`, 17.09) СНЯЛ ДОВОД, А НЕ ДОБАВИЛ ШАГ:**
  /// второго вида события не существует, значит и второго документа под
  /// позванного быть не должно. Приглашённый — участник ТОГО ЖЕ вечера:
  /// `musicians` плюс ключ в `answers`. Так это устроено в принятой практике —
  /// RFC 5545 держит `ATTENDEE` с `PARTSTAT` внутри самого события, Google
  /// Calendar API — `attendees[].responseStatus` (I69, пункт 1).
  ///
  /// **ЧТО ИЗ ЭТОГО СЛЕДУЕТ ПРЯМО ЗДЕСЬ: ПОРЯДОК ЗАПИСЕЙ ПЕРЕСТАЛ
  /// СУЩЕСТВОВАТЬ.** Запись одна, значит «оборвалось посередине» невозможно
  /// по устройству: либо позвали и записали шаблон, либо не случилось ничего.
  /// Прежний разбор порядка снят вместе с самим порядком, а не забыт.
  ///
  /// **ИМЯ ПОСТУПКА — `edited`, И ЭТО ТРЕБОВАНИЕ, А НЕ УМОЛЧАНИЕ.** От него
  /// зависит, узнает ли позванный о зове:
  ///
  ///   • `planUpdatePushes` ветвится ПО ИМЕНИ, и восемь имён выходят из неё
  ///     раньше разбора состава (`ownerCancelled`, `ownerDoubt`, `ownerFirm`,
  ///     `restored`, четыре хода отмены). Поставь любое из них — и
  ///     `diff.added` не будет прочитан вовсе, то есть «Tədbirə əlavə
  ///     olundunuz» не уйдёт НИКОМУ. Молчание неотличимо от «нас не звали»;
  ///   • `edited` проваливается в `default`, оттуда разбирается состав, и
  ///     позванный получает то же письмо, что получал на своём документе;
  ///   • `pushEdited` при этом не уйдёт: он стоит за `diffEvents`, где
  ///     сравниваются дата, место и заметки, — а мы их не трогаем.
  ///
  /// **ПОДПИСЬ ОБЯЗАТЕЛЬНА ПО ТОЙ ЖЕ ПРИЧИНЕ.** `lastActionBy` — то, откуда
  /// сервер берёт автора: `recipientsOf` снимает его из адресатов, а
  /// `displayName` ставит его имя в текст. Без подписи имя осталось бы от
  /// ПРОШЛОГО поступка (I54), и письмо назвало бы постороннего.
  ///
  /// **ВЫКЛАДКА ПРАВИЛ ЭТОМУ ШАГУ НЕ НУЖНА, и это проверяемо, а не обещано.**
  /// Ветвь владельца в `allow update` перечисления ключей не имеет; запрещены
  /// ей только имена отмены (`namesCancelDeed` — четыре слова, `edited` среди
  /// них нет) и повод состояния (`serverOwnsUnsettledReason`). `stampsSelf()`
  /// выполнен подписью самим собой, `stampsTime()` — серверной отметкой.
  ///
  /// **[added] — ТОЛЬКО НОВЫЕ, а прежний состав берётся из [event].** Слить их
  /// у вызывающего значило бы дать ему решать, кто в составе «был», — а от
  /// этого зависит перенос ответов (N114, N173): ответивший обязан ответ
  /// сохранить, а ушедший и позванный заново — отвечать заново.
  ///
  /// **СЫРАЯ КАРТА ОТВЕТОВ БЕРЁТСЯ ЗДЕСЬ, И ЭТО ВТОРОЕ ЕЁ МЕСТО ИЗ ДВУХ.**
  /// Первое — правка состава (`agreements_screen.dart`). Оба перезаписывают
  /// карту ЦЕЛИКОМ по составу, то есть оба — законные писатели, ради которых
  /// узкий вход и заведён. Сторож по исходникам держит теперь **набор мест, а
  /// не их число** (I13: состав, а не количество), иначе третий вызов сошёл бы
  /// за один из этих двух.
  /// **[slots] — ВЕСЬ СПИСОК ОТПРАВКИ, а в документ уходит только его часть,
  /// и отбор делает ЭТА функция, а не вызывающий (17.09, вариант Б).**
  ///
  /// **ПОЧЕМУ ЗДЕСЬ, А НЕ У ВЫЗЫВАЮЩЕГО — ЭТО ЗАМЕР, А НЕ ВКУС.** Первая
  /// редакция отбирала на стороне экрана и держалась вердиктом по исходникам:
  /// «в поле уходит `notInvited`». Порча показала, что вердикт слеп — он
  /// смотрит на имя параметра в теле писателя, а подменить можно то, что в
  /// него ПЕРЕДАЮТ. Порченый вызов прошёл зелёным (I55: портить надо
  /// настоящий путь, а не ближайшую точку входа).
  ///
  /// Отбор внутри делает нарушение невозможным по устройству: что бы ни
  /// прислал вызывающий, слот `invited: true` в документ не попадёт.
  /// Устройство сильнее сторожа — сторож напоминает, устройство не пускает.
  ///
  /// **Что это охраняет:** позванный лежит в `musicians`, и второй его след в
  /// поле непозванных дал бы вторую строку на экране — дефект 17.09 дословно.
  ///
  ...
  Future<void> callPeopleToEvent({
    ...
    batch.update(eventRef, {
      ...
      kAnswersWrittenByOwner: true,
      // НЕПОЗВАННЫЕ — ТОЙ ЖЕ ЗАПИСЬЮ. Поле отвечает на то, чего в составе нет
      // и быть не может: «кого хотели позвать и почему не ушло». Позванный
      // виден и без него — он в составе; непозванный не виден больше нигде,
      // и держать его в `musicians` нельзя (правило чтения пустило бы его в
      // чужой вечер, а сервер послал бы ему «вас добавили»).
      //
      // Список перезаписывается целиком, а не дополняется: он и есть ответ на
      // вопрос «кому не ушло в этот раз», а дописывание превратило бы его в
      // историю, у которой нет ни одного читателя.
      'lineup': lineupToFirestore(
        slots.where((s) => !s.invited).toList(),
      ),
      'lastActionBy': callerUid,
      // ЛИТЕРАЛОМ, А НЕ ЧЕРЕЗ `kEventEdited`, И ЭТО НЕ НЕБРЕЖНОСТЬ (I31).
      // Имя поступка сторожится проходом по исходникам, а проход читает
      // литералы: вынеси значение за константу — и сторож ослепнет, причём
      // молча, потому что утверждает он ОТСУТСТВИЕ запрещённых имён. Тем же
      // литералом и по тому же доводу пишут соседи (`addPersonalEvent`) и
      // словарь поступков (`event_deed_line.dart`). Значение обязано
  ...
  /// Выйти из ЧУЖОГО мероприятия — **ответом за себя, а не правкой состава**
  /// (N121, шаг 2; переписано 29.08).
  ///
  /// Не удаление: в мероприятии заняты и другие люди, у них оно остаётся. Из
  /// календаря вышедшего оно пропадает — но теперь **не потому, что человека
  /// не стало в данных, а потому что так решает показ** (`showsInCalendarOf`
  /// в `core/agreements/day_role.dart`, разбор там же).
  ///
  /// --- ЧТО ЗДЕСЬ БЫЛО И ПОЧЕМУ УШЛО ---
  ///
  /// Было: `musicians: FieldValue.arrayRemove([uid])` плюс `lastActionBy` и
  /// `lastActionType: 'left'` — правило `leavesEvent()`, три ключа. Уход
  /// **физически вычёркивал человека из состава**, и потому:
  ///
  ///   • владелец переставал видеть, КТО ушёл — в составе оставался он один;
  ///   • пометка «İşdən çıxdı» показать было некому и нечему: строки в
  ///     блоке состава больше не существовало. Кнопка «?» у вышедшего не
  ///     показывалась никому и никогда — это и есть **N121**;
  ///   • вечер вставал «под вопрос» (`unsettledAfterMemberLeft`), хотя под
  ///     вопросом один человек, а не вечер (решение 12.08).
  ///
  /// Стало: **один ключ `answers.<uid> = 'left'`**, правило
  /// `answersForSelf()`. Человек ОСТАЁТСЯ в составе — под именем стоит
  /// «İşdən çıxdı», — а из календаря вечер убирает показ.
  ///
  /// **Права при этом не расширены ни на йоту:** `answersForSelf()` выложен
  /// 12.08 и уже разрешал значение `'left'`; участник и вчера был вправе
  /// записать его себе. Наоборот — `leavesEvent()` стало мёртвым, и снятие
  /// его было **сужением** прав. **Шаг 3 сделан 30.08: правила больше не
  /// пускают правку `musicians` участником вовсе.**
  ///
  /// --- ТРИ ЦЕНЫ, НАЗВАННЫЕ ДО КОДА (N182) ---
  ///
  /// **1. Строка «что случилось» про уход ЗАМОЛЧИТ, и это не поломка.**
  /// `answersForSelf()` — `hasOnly(['answers'])`, значит `lastActionType`
  /// вторым ключом не пройдёт, и правило отвергнет запись ЦЕЛИКОМ. Расширить
  /// его нельзя по I54: это отдало бы участнику поле, по которому **сервер
  /// судит о смысле произошедшего**, и «ответ» с приписанным `'left'` стал бы
  /// в глазах сервера уходом. Владелец узнаёт об уходе двумя признаками
  /// вместо трёх: push (`pushLeft`, текст прежний) и пометка в составе.
  ///
  /// **2. Записанное клиентом не снимается клиентом.** `answersForSelf()`
  /// требует `uid in request.resource.data.answers` — стереть свой ключ
  /// участник не вправе, и условие стоит по делу: иначе он отменил бы сам
  /// факт вопроса. Убрать `'left'` может только владелец, вынув человека из
  /// состава и вернув обратно.
  ///
  ...
  /// «уход перестал быть событием вечера и **стал ответом человека**». Раз
  /// так, писать его обязан тот же ход, что пишет остальные четыре: одно
  /// правило, один путь, одна форма записи `answers.<uid>`. Расхождения нет
  /// ни в чём — ни в ключе, ни в правиле, ни в обращении с отказом, — значит
  /// общей становится вся функция, а не часть.
  ///
  /// **Имя всё же своё, и это не украшение.** У места вызова поступок зовётся
  /// «выйти», а не «ответить»; на нём стоит сторож
  /// (`source_invariants_test.dart`, «вызов leavePersonalEvent из формы не
  ...
  /// перезаписью карты. Это не бережливость: правило шага 4 разрешает участнику
  /// `diff().affectedKeys().hasOnly([request.auth.uid])`, и карта целиком —
  /// это отказ по правам, даже если чужие значения в ней не изменились.
  ///
  /// **Точка с запятой в пути значима:** `answers.$uid` в Firestore — это
  /// правка вложенного поля, а `{'answers': {...}}` — замена всей карты.
  /// Вторая форма стёрла бы ответы остальных.
  ///
  /// **ОТКАЗ НЕ ГЛОТАЕТСЯ.** До выкладки правил (пункт 1) участник получит
  /// здесь `permission-denied` — сегодня `personalEvents` разрешает `update`
  /// только владельцу. Это не поломка, а порядок работ: клиент пишется
  /// раньше, правила выкладываются последними, одним заходом с прогоном. Молча
  /// проглоченный отказ выглядел бы как «ответ принят», и человек ушёл бы
  /// уверенным, что сказал «не могу», не сказав ничего.
  Future<void> setEventAnswer(String eventId, String uid, String answer) {
    return _db
        .collection('personalEvents')
        .doc(eventId)
        .update({'answers.$uid': answer}).timeout(_writeTimeout);
  }
...
/// Просмотренные приглашения — множество id мероприятий (12.08).
///
/// **Не `autoDispose`**, в отличие от соседей выше: набор нужен календарю при
/// каждом показе месяца, и пересоздавать подписку на каждый вход значило бы
/// показывать полное число на секунду при каждом открытии вкладки — то самое
/// мигание, которое человек прочтёт как «пришли новые приглашения».
final seenInvitationsProvider = StreamProvider.family<Set<String>, String>((
  ref,
  uid,
) {
  return ref.watch(firestoreServiceProvider).watchSeenInvitations(uid);
});
