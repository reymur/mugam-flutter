...
class FirestoreService {
  ...
  Future<void> callPeopleToEvent({
    ...
    await _db.collection('personalEvents').doc(event.id).update({
      ...
      kAnswersWrittenByOwner: true,
      // ШАБЛОН — ТОЙ ЖЕ ЗАПИСЬЮ. Он отвечает на то, чего в составе нет:
      // «кого хотели позвать, и кому не ушло». Позванный виден и без него —
      // он в составе; непозванный не виден больше нигде.
      //
      // Список перезаписывается целиком, а не дополняется: он и есть ответ на
      // вопрос «кого звали в этот раз», а дописывание превратило бы его в
