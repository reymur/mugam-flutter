...
void main() {
  ...
  group('вечер — одиннадцать типов ведут в САМ вечер', () {
    const eventTypes = [
      // Заведён 11.09 починкой N213: владелец отменил личный вечер.
      // Отменённый вечер по правилам читается, дверь его находит — значит
      // в карточку, а не в список.
      'event_cancelled',
      // Заведён 11.09 починками N214 и N216: вечер вернули в силу.
      // ОДИН тип на два поступка — у них один ответ адресату и одна дверь.
      'event_back_in_force',
  ...
  group('правило не сводится к одному ответу', () {
    ...
    test('разбираемых типов ровно двадцать два', () {
      ...
      // Двадцать первый — `event_cancelled`, 11.09, починка N213;
      // двадцать второй — `event_back_in_force`, тем же днём, N214 и N216.
      expect(kKnownPushTypes.length, 22);
    });

    // ДВЕ СТОРОНЫ ГРАНИЦЫ СВЕРЯЮТСЯ ПОИМЁННО, И ЭТО ЗАВЕДЕНО 11.09 ВМЕСТЕ С
    // ПОЧИНКОЙ N213 — ПОТОМУ ЧТО ОНА ЗАВОДИТ НОВЫЙ ТИП, А НОВЫЙ ТИП И ЕСТЬ
    // ЕДИНСТВЕННЫЙ СПОСОБ ПОВТОРИТЬ N59.
    //
    // Сверка поимённая, а не по числу (I13): недосчитать список незаметно
    // нельзя, он сам называет, кого не хватает.
    //
    // **ДВЕ СТОРОНЫ, И ОБЕ НАСТОЯЩИЕ.** Тип, который шлёт сервер, а клиент
    // не разбирает, — это мёртвое нажатие (N59, девять типов разом).
    // Тип, который клиент разбирает, а никто не шлёт, — мёртвый код, и он
    // опаснее, чем кажется: он выглядит доказательством, что дорога есть.
    //
    // **УТВЕРЖДЕНИЕ НАЛИЧИЯ, ЗНАЧИТ САМО СЕБЕ КАНАРЕЙКА (I31):** ослепни
    // разбор — множество серверных типов опустеет, и первая же половина
    // покраснеет, назвав все двенадцать имён.
    //
    // ЧЕГО НЕ ЛОВИТ: он читает ОДИН файл сервера — событийные типы живут
    // там все, а чатовые (`offerMoves.ts`, `index.ts`) сверяются соседкой
    // ниже и вручную.
    test('событийные типы совпадают с отправителем в обе стороны', () {
      final src =
          File('functions/src/eventNotifications.ts').readAsStringSync();
      final sent = RegExp(r'"(event_[a-z_]+)"')
          .allMatches(src)
          .map((m) => m.group(1)!)
          .toSet();
      final known =
          kKnownPushTypes.where((t) => t.startsWith('event_')).toSet();

      expect(sent, isNotEmpty, reason: 'разбор ослеп: сервер шлёт ноль типов');
      expect(known.difference(sent), isEmpty,
          reason: 'клиент разбирает тип, которого никто не шлёт');
      expect(sent.difference(known), isEmpty,
          reason: 'сервер шлёт тип, по которому нажатие не ведёт никуда');
    });
