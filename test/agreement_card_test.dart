...
void main() {
  ...
  group('исход: под вопросом (Часть 6а)', () {
    // Третье состояние пишет только сервер: у договора пропала опора —
    // ушёл участник либо исчезла работа.
    test('под вопросом — свой исход, а не «в силе»', () {
      final card = state(status: kEventStatusUnsettled);
      expect(card.outcome, AgreementOutcome.unsettled);
    });

    // ПОРЯДОК РАЗБОРА, и он не косметика: у договора под вопросом может
    // стоять и запрос отмены. Первым человек обязан увидеть, что опоры
    // нет, — иначе ответит на запрос, не зная, что работы больше нет.
    test('под вопросом старше стоящего запроса отмены', () {
      final card = state(
        status: kEventStatusUnsettled,
        requestedBy: 'him',
        lastBy: 'him',
        lastType: 'cancelRequested',
      );
      expect(card.outcome, AgreementOutcome.unsettled);
    });

    // Отменённость всё же старше: у отменённого поля запроса не
    // очищаются, и разбор по ним увидел бы вопрос там, где всё кончилось.
    test('отменённость старше «под вопросом»', () {
      final card = state(status: 'cancelled');
      expect(card.outcome, AgreementOutcome.cancelledByAgreement);
    });

    // Соседка: обычный договор своего исхода не меняет. Без неё правило,
    // отвечающее «под вопросом» всегда, было бы зелёным на трёх тестах
    // выше.
    test('обычный договор остаётся в силе', () {
      expect(state().outcome, AgreementOutcome.active);
    });
  });
