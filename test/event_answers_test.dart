...
void main() {
  group('шаг 1 · какой answers соответствует составу', () {
    ...
    test('N114: ВЫШЕДШИЙ, приглашённый заново, отвечает ЗАНОВО', () {
      // Его ключ остался в карте после выхода — `leavesEvent` (снято 30.08) к
      // `answers` не пускало. Без учёта прежнего состава прежний `going` переносится
  ...
  group('шаг 2 · расхождение записи с составом — для переписи, не для экрана', () {
    ...
    test('ИЗБЫТОК: ключ вне состава — ожидаемо от выхода из состава', () {
      // `leavesEvent()` разрешало трогать только `musicians`, `lastActionBy`
      // и `lastActionType`; ключ ушедшего в карте остаётся. Правило снято
      // 30.08 — записи, которые оно наделало, остались.
      final m = answersMismatch(
  ...
  group('шаг 1 · чтение, которое появится на шаге 3', () {
    ...
    test('КЛЮЧ ВНЕ СОСТАВА НЕ СЧИТАЕТСЯ — решение 10.08, не находка', () {
      ...
      // `leavesEvent()` разрешало трогать только `musicians`, `lastActionBy`
      // и `lastActionType`, а `answers` в этот набор не входил. Значит
      // лишний ключ — мусор, и спрашивать надо сначала состав. Правило снято
      // 30.08, разбор остаётся: старые ключи вне состава никуда не делись.
      expect(
        answerOf(
          uid: 'ушедший',
          participantUids: const ['a'],
