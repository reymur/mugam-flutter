...
void main() {
  ...
  group('сторож: решение про историю принимает один виджет', () {
    ...
    test('ДОЛЯ ЧИСЛОМ: одиннадцать мест зовут ободок', () {
      ...
      //
      // **ПОТЕРЯ ПРИНЯТА ВЛАДЕЛЬЦЕМ 19.09: ободок в окне выбора людей не
      // нужен, оставляем как есть.** Записано решением, а не умолчанием —
      // иначе следующий, увидев список без ободка, прочтёт это как недосмотр
      // и «починит», отдав ободок заодно одноместному листу «кому предложить
      // работу», где его отродясь не было.
      //
      // ТО ЕСТЬ ЭТО НЕ ДОЛГ И НЕ НАХОДКА. Возвращать нечего: девять мест, где
      // ободок про историю, работают как прежде, а в окне выбора он и не
      // спрашивался — там выбирают человека, а не смотрят его историю.
      //
  ...
  // =======================================================================
  // СВОЙ ПРОФИЛЬ: ОБОДОК ЕСТЬ ТОЛЬКО ПРИ ЖИВОЙ ИСТОРИИ (19.09, N251-вторая)
  //
  // ПОВОД ЧИСЛОМ, А НЕ СЛОВАМИ. Найдено владельцем на трубке 19.09: на своём
  // профиле было НАОБОРОТ — нет истории золотой ободок, есть история
  // приглушённый. Обход одиннадцати мест показал, что сломано ровно одно:
  // `profile_screen.dart` задавал `plainRingColor: kGold` БЕЗУСЛОВНО (золото
  // в ветви «истории нет» есть ещё у четверых, но у всех под условием —
  // `selected` либо `goldRing`) и подавал `currentUid: user.id`, то есть
  // делал «это я» тождеством. Первое красило пустую ветвь ярче полной,
  // второе не давало полной ветви стать золотой никогда.
  //
  // ПОЧЕМУ ВЕРДИКТЫ ЗДЕСЬ, А НЕ ПО ИСХОДНИКУ. Правило про то, что ВИДНО, а не
  // про то, как записано: между `hasUnviewed` и рамкой стоит ещё одно
  // правило (`ringColor ?? …` в `AvatarRing`), и разбор исходника его не
  // проходит. `_ringColor` спрашивает саму нарисованную рамку.
  //
  // ЧЕГО ЭТИ ВЕРДИКТЫ НЕ ЛОВЯТ: что `profile_screen.dart` зовёт `StatusRing`
  // именно с этими параметрами. Это сторож по исходнику ниже, в той же
  // группе, и он назван отдельно — вердикт о виджете и вердикт о вызывающем
  // отвечают на разные вопросы (I13).
  // =======================================================================
  group('свой портрет: ободок только при живой истории', () {
    // КАК СВОЙ ПРОФИЛЬ ЗОВЁТ ОБОДОК — один в один с `profile_screen.dart`:
    // ширина пустой ветви ноль, цвет пустой ветви НЕ задан, смотрящий и
    // показываемый — один человек.
    Future<void> showOwn(WidgetTester tester, {required User? me}) => _show(
          tester,
          user: me,
          currentUid: 'me',
          viewer: me,
    ...
    testWidgets('НЕТ истории — ободка нет вовсе', (tester) async {
      await showOwn(tester, me: _u(id: 'me'));
      expect(_ringColor(tester), isNull,
          reason: 'на своём портрете без живой истории нарисован ободок. '
              'Решение владельца 19.09: нет истории — нет ободка. Золотой '
              'здесь особенно неверен: во всём приложении золото значит '
              '«есть что посмотреть впервые»');
    });

    testWidgets('ЕСТЬ история — ободок есть', (tester) async {
      await showOwn(
        tester,
        me: _u(
          id: 'me',
      ...
      expect(_ringColor(tester), isNotNull,
          reason: 'своя живая история не показана ободком вовсе');
    });

    testWidgets('своя история — ПРИГЛУШЁННЫЙ, а не золотой', (tester) async {
      // Своя история просмотрена по определению: её только что выложил тот,
      // кто смотрит. Из двух цветов ободка остаётся ровно один.
      await showOwn(
        tester,
        me: _u(
          id: 'me',
      ...
      expect(_ringColor(tester), kMuted,
          reason: 'своя история светится золотом — значит правило «у себя '
              'ободок не золотой» обойдено');
    });

    testWidgets('ИСТЁКШАЯ своя история — ободка нет', (tester) async {
      // Третий случай, и он не лишний: поле `mostRecentStatusExpiresAt`
      // сервер никогда не чистит, поэтому «была история» и «есть история»
      // различимы только сравнением со временем.
      await showOwn(
        tester,
        me: _u(
          id: 'me',
          storyExpiresIn: const Duration(hours: -1),
          storyPostedAgo: const Duration(hours: 25),
        ),
      );
      expect(_ringColor(tester), isNull,
          reason: 'истёкшая своя история всё ещё рисует ободок');
    });

    // -------------------------------------------------------------------
    // КАНАРЕЙКА. Три вердикта выше держатся на `_ringColor(...) == null`,
    // то есть на УТВЕРЖДЕНИИ ОТСУТСТВИЯ, а оно ломается молча (I31):
    // перестань `_ringColor` находить рамку — он вернёт `null`, `null` и
    // есть искомое, и вердикты зазеленеют собственной слепотой.
    //
    // Канарейка ищет ТЕМ ЖЕ `_ringColor`, в том же виджете, заведомо
    // существующую рамку — чужую непросмотренную историю, которая обязана
    // быть золотой.
    // -------------------------------------------------------------------
    testWidgets('КАНАРЕЙКА: тот же способ ВИДИТ рамку, когда она есть',
        (tester) async {
      await _show(
        tester,
        user: _u(
      ...
      expect(_ringColor(tester), kGold,
          reason: 'способ чтения рамки ослеп: он не находит ЗАВЕДОМО '
              'существующий золотой ободок чужой непросмотренной истории. '
              'Значит и «ободка нет» он доказать не может');
    });

    // -------------------------------------------------------------------
    // СТОРОЖ ПО ИСХОДНИКУ — отдельным вердиктом, потому что вопрос другой:
    // выше проверено, ЧТО РИСУЕТ виджет, здесь — С ЧЕМ ЕГО ЗОВЁТ свой
    // профиль. Зелень одного ничего не говорит о другом (I13).
    // -------------------------------------------------------------------
    test('свой профиль зовёт ободок без золота и без ширины', () {
      final code = readCode('lib/features/profile/screens/profile_screen.dart');
      // Канарейка к срезу: вызов в файле вообще есть.
      expect('StatusRing('.allMatches(code).length, 1,
          reason: 'свой профиль перестал звать общий ободок — значит и '
              'проверять ниже нечего');
      expect(code.contains('plainRingWidth: 0'), isTrue,
          reason: 'у своего портрета вернулась ширина ободка в ветви '
              '«истории нет». Решение владельца 19.09: нет истории — нет '
              'ободка');
      // С ДВОЕТОЧИЕМ — то есть МЕСТО ПЕРЕДАЧИ параметра, а не голое имя.
      // Сторож над сторожами поймал первую редакцию и был прав: имя лежит в
      // файле всегда, передачи может уже не быть.
      expect(code.contains('plainRingColor:'), isFalse,
          reason: 'у своего портрета снова задан цвет пустой ветви. Он '
              'снят СОВСЕМ, а не выставлен в ноль ширины, нарочно: при '
              'нулевой ширине цвет не рисуется, но остаётся заряженной '
              'ловушкой — вернёт кто-нибудь ширину, и золото придёт '
              'обратно молча');
      expect(code.contains('currentUid: user.id'), isFalse,
          reason: 'смотрящий снова выводится из показываемого. Тогда «это '
              'я» становится тождеством, а не проверкой: покажи шапку для '
              'другого человека — его непросмотренная история погаснет');
    });
  });
}
