...
// документа. Проверяется не поведением экрана, а АРИФМЕТИКОЙ: 9
// переписываемых плюс 13 сохраняемых обязаны дать ровно 22 ключа
...
// другую.
//
// СЧЁТ ИЗМЕНИЛСЯ ВТОРИЧНО 11.08 — 8→9 и 21→22, `answersWrittenByOwner`
// (починка N115: «ключа нет» значит «не спрашивали» только тогда, когда карту
// заполнял тот, кто пишет её целиком). И арифметика снова сработала как
// задумана — четыре проверки покраснели сами, до того как я о них вспомнил.
//
...
void main() {
  group('N40 · арифметика полей', () {
    ...
    test('поимённо: что правка переписывает', () {
      ...
      // замысла: вместо одного поля теперь сторожатся все девять.
      expect(kEventEditWrites, {
        ...
        'musicians',
        'answers',
        // N115: отметка едет ВМЕСТЕ с картой и той же записью. Разъедься они
        // — и появился бы документ с полной картой без отметки, то есть
        // ровно тот случай, который отметка и заведена различать.
        'answersWrittenByOwner',
        'lastActionBy',
        // ОТМЕТКА ВРЕМЕНИ ПОСТУПКА — 09.09, N209. Правка её ПЕРЕПИСЫВАЕТ,
        // хотя `eventEditUpdate` её не возвращает: добавляет
        // `FirestoreService.updatePersonalEvent`, потому что
        // `serverTimestamp()` втащил бы в чистое правило зависимость от базы
        // (N181). Сторона выбрана в пользу правды о документе — разбор в
        // шапке самого набора.
        'lastActionAt',
        'lastActionType',
      });
    ...
    test('поимённо: что правка обязана сохранить', () {
      ...
      expect(kEventEditPreserves, {
        ...
        'unsettledReason',
        // ПРИГЛАШЕНИЯ СВОИХ — 09.09, работа 7, шаг 5. `parentEventId`:
        // перепиши его правка, и ребёнок потерял бы родителя либо приклеился
        // к чужому вечеру. `lineup`: пересборка при чтении обновляла бы
        // запасное имя ровно тогда, когда человека в живом списке уже нет
        // (I9), и стёрла бы отметку непозванного (I47).
        'parentEventId',
        'lineup',
        'createdAt',
      });
  ...
  group('N40 · сама запись', () {
    ...
    test('пишет ровно девять из десяти, и десятый назван поимённо', () {
      ...
      // один ключ: правка его ПЕРЕПИСЫВАЕТ, но ставит его не чистое правило,
      // а `FirestoreService.updatePersonalEvent` — `serverTimestamp()`
      // втащил бы в правило зависимость от базы (N181).
      //
      // Ключ назван ЗДЕСЬ ИМЕНЕМ, а не вычтен числом: «на один меньше»
      // истинно и тогда, когда пропал не тот ключ (I13 — состав, а не
      // количество).
      const stampedByService = 'lastActionAt';
      final data = update();
      expect(
        data.keys.toSet(),
        kEventEditWrites.difference({stampedByService}),
        reason: 'Чистое правило вернуло не тот набор. Оно обязано вернуть '
      ...
      expect(kEventEditWrites.contains(stampedByService), isTrue,
          reason: 'канарейка: отметка ушла из переписываемых — вычитание '
              'выше перестало что-либо значить');
      // И вторая половина: отметку ставит писатель, а не никто. Вердикт на
      // самого писателя стоит в `test/last_action_at_wiring_test.dart`.
      expect(data.containsKey('lastActionType'), isTrue,
          reason: 'канарейка: правка перестала называть поступок, а значит '
              '`updatePersonalEvent` не поставит и отметку времени');
    });
