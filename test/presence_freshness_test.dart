...
void main() {
  ...
  // isActuallyOnline обслуживает ТРИНАДЦАТЬ кружков в ОДИННАДЦАТИ файлах, и
  // теперь это один виджет, а не тринадцать рукописных: счёт снят 18.09
  // командой `grep -rn "OnlineDot(" lib`, он же стоит сторожем в
  // `online_dot_test.dart`. Здесь стояло «12 ФАЙЛОВ» — верное на тот час
  // число ЧИТАТЕЛЕЙ правила, а не мест с кружком; после сведения считать
  // надо зовущих (I6: у числа стоит либо дата, либо команда).
  //
  // Кружок обязан судить по тому же окну, что и строка в плашке переговоров
  // под ним: при разных окнах два индикатора на одном экране начинают
