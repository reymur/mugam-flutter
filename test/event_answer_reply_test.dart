...
void main() {
  ...
  group('N39: набор ответов не зависит от владения мешающим вечером', () {
    ...
    test('ЗАКРЫТИЕ ОКНА и «не смогу» — РАЗНЫЕ исходы, хоть оба молчат (I47)', () {
      // ЗДЕСЬ БЫЛО ОБРАТНОЕ: вердикт требовал, чтобы они РАЗЛИЧАЛИСЬ ответом —
      // закрытие давало `null`, «не могу» давало `cant`. С 17.09 отказ не
      // пишет значения вовсе: он открывает лист причины. Оба выбора теперь
      // молчат, и различать их по возвращённому ответу нечем.
      //
      // ЭТО НЕ ПОТЕРЯ РАЗЛИЧЕНИЯ, А ПЕРЕЕЗД ЕГО В ДРУГОЕ МЕСТО. Правило
      // отвечает на «что записать», и честный ответ у обоих — «ничего».
      // Различает их вызывающий, по самому выбору, и ровно так же он с самого
      // начала различал `view`.
      expect(answerAfterConflict(null), isNull);
      expect(answerAfterConflict(AnswerConflictChoice.cannotGo), isNull);
      // И ГЛАВНОЕ, ЧТО ОСТАЛОСЬ ВЕРНЫМ: закрытие окна не должно писать ответа
      // НИКОГДА. Пусть исходы сравнялись — этот запрет держится сам по себе.
      expect(
        answerAfterConflict(null),
        isNot(kAnswerGoing),
        reason: 'Закрытие окна пишет согласие. Человек закрыл окно, а вечер '
            'записал, что он придёт.',
      );
    });
    ...
    test('ЕДИНСТВЕННЫЙ ПИШУЩИЙ ВЫБОР — «всё равно иду»', () {
      // ЗДЕСЬ БЫЛО «оба пишущих ответа пишут РАЗНОЕ»: их было два, `going` и
      // `cant`. С 17.09 пишущий один — отказ идёт листом причины, а не
      // записью значения. Вердикт переписан под это и утверждает НАЛИЧИЕ
      // (I31): ослепни правило — покраснеет, а не позеленеет.
      final go = answerAfterConflict(AnswerConflictChoice.goAnyway);
      expect(go, kAnswerGoing);
      expect(kEventAnswers.contains(go!), isTrue,
          reason: 'Окно пишет ответ вне известного набора — тогда его не '
              'поймут ни показ, ни сервер, ни правила.');

      // КАНАРЕЙКА К ТРЁМ МОЛЧАНИЯМ ВЫШЕ. Три вердикта подряд ждут `null`, и
      // все три зазеленели бы разом, начни правило молчать на всём подряд.
      expect(answerAfterConflict(AnswerConflictChoice.goAnyway), isNotNull,
          reason: 'канарейка: правило молчит на ВСЕХ выборах — значит '
              'ожидания `null` выше ничего не доказывают');
    });
