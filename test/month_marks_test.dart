...
void main() {
  ...
  group('приглашение помечает день, но НЕ занимает его', () {
    ...
    // ЗДЕСЬ БЫЛ ВЕРДИКТ «ОТКАЗ — день НЕ занят, но след остаётся серой
    // точкой» (N126, поправлен 12.08 по виду на трубке). СНЯТ 17.09 вместе с
    // ответом `cant`.
    //
    // ЧТО ЭТИМ ПОТЕРЯНО, НАЗВАНО ПРЯМО: серого следа у отказавшегося больше
    // нет. Отказ стал ВЫХОДОМ, а вышедшему вечер не показывается вовсе
    // (`showsInCalendarOf`, решение 29.08) — клетка у него пуста, и это не
    // дефект, а то же решение с другой стороны: он ушёл из дела, а не
    // разобрал вопрос.
    //
    // Единственный оставшийся след — просмотренное приглашение без ответа, и
    // его держит вердикт ниже.
    test('ПРИГЛАШЕНИЕ БЕЗ ОТВЕТА — день НЕ занят, и на клетке число', () {
      // ЧТО ОСТАЛОСЬ ОТ ПРЕЖНЕГО ВЕРДИКТА: половина, не зависевшая от `cant`, —
      // неотвеченный вопрос день НЕ занимает. Это N126 в своей первой
      // половине, и она верна по-прежнему.
      final mark = dayMarkOf(
          [invitation('2026-08-14T20:00:00.000', answer: 'waiting')], _names,
          currentUid: _me);
      expect(mark, isNotNull);
      expect(mark!.unseenInvitations, 1,
          reason: 'неотвеченное ждёт просмотра — золотое число');
      expect(mark.occupied, isFalse, reason: 'вопрос день не занимает');
      expect(mark.invited, isTrue, reason: 'вопрос ещё не разобран');
      expect(mark.ownerUid, isNull);
      expect(mark.shape, isNull);
    });
    ...
    test('ЧИСЛО СТАРШЕ СЛЕДА: одно приглашение и один отказ в один день', () {
      ...
      final mark = dayMarkOf([
        invitation('2026-08-14T18:00:00.000', answer: 'left'),
    ...
    test('СОСЕДКА-КАНАРЕЙКА: чужими глазами тот же день читается иначе', () {
      ...
      // ВХОД СМЕНЁН 17.09: был отказавшийся (`cant`), стал ПОЗВАННЫЙ БЕЗ
      // ОТВЕТА. Работа канарейки та же — один вход, три разных ответа трём
      // парам глаз, — а прежний вход исчез вместе с ответом: вышедшему вечер
      // не показывается вовсе, и «свободен со следом» у него взять неоткуда.
      final e = invitation('2026-08-14T20:00:00.000', answer: 'waiting');
      // Владелец — день занят.
      final owner = dayMarkOf([e], _names, currentUid: 'raf');
      expect(owner!.occupied, isTrue);
      expect(owner.ownerUid, 'raf');
      // Позванный — день свободен, но на клетке число.
      final me = dayMarkOf([e], _names, currentUid: _me);
      expect(me!.occupied, isFalse);
      expect(me.invited, isTrue);
      expect(me.unseenInvitations, 1);
