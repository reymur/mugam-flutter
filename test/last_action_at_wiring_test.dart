...
void main() {
  group('отметка времени поступка проведена до писателей (N181)', () {
    ...
    test('КАНАРЕЙКА: писатели поступка вообще находятся', () {
      ...
      // ДЕСЯТЫЙ ПИСАТЕЛЬ ЗАВЕДЁН 09.09 — `createLineupInvitation`,
      // приглашение своего в состав (работа 7, шаг 5). Число поднято с
      // девяти РОВНО НА ЭТО, и отметку он ставит рядом, как требует
      // вердикт ниже. Поднимать число, не заведя отметку, нельзя: канарейка
      // тогда молчала бы о настоящем нарушении.
      expect(
        total,
        10,
        reason: 'писателей поступка стало $total вместо десяти — '
