...
void main() {
  ...
  group('предложение работы создаётся одним кодом (пункт 6)', () {
    ...
    // «е»: ИМЯ ПРЕДЛОЖИВШЕГО БЕРЁТСЯ У ЖИВОГО ПОЛЬЗОВАТЕЛЯ, А НЕ У ПОЛЯ ЧАТА.
    //
    // **Поймано на трубке 25.08, в первый же показ подписи (N164):** вышло
    // «Naməlum təklif edir». Настоящая цена больше — `chat_screen.dart:4418`
    // записывает прямо, что **mugam-v2 пишет `name` прямого чата со стороны
    // СОЗДАТЕЛЯ и никогда не обновляет**, то есть с непустым полем строка
    // назвала бы предложившим самого читателя.
    //
    // Сторож текстовый, потому что место живёт внутри состояния экрана и в
    // тесте не поднимается: чат тянет Firestore, Auth и роутер.
    //
    // --- ЧЕГО НЕ ЛОВИТ, И ГЛАВНОЕ ЗДЕСЬ ПЕРВОЕ (I50) ---
    //
    // **1. ВТОРОГО ЧИТАТЕЛЯ ТОГО ЖЕ НЕГОДНОГО ПОЛЯ — `_replySenderName`.** Он
    // берёт имя оттуда же и **записывает его в документ сообщения**
    // (`replyTo.senderName`), то есть закрепляет неправду в данных. Это
    // **известно и отложено решением владельца 25.08** (N141), а не забыто:
    // отдельная работа, отдельная переменная. Сторож молчит о нём намеренно —
    // распространи его сюда сейчас, и он покраснел бы на коде, который решено
    // не трогать.
    //
    // 2. Что `_otherUserCached` заполнен верно — вопрос к `currentUserProvider`.
    // 3. Подпись СВОЕЙ стороны: она идёт от `FirebaseAuth`, подменить нечем.
    test('е: имя предложившего берётся у живого пользователя', () {
      const chat = 'lib/features/chat/screens/chat_screen.dart';
      final lines = readCode(chat).split('\n');

      expect(
        lines.where((l) => l.contains('offerInitiatorName =')).length,
        1,
        reason: 'подпись автора предложения собирается не в одном месте — '
            'значит источников имени снова несколько',
      );

      final at = lines.indexWhere((l) => l.contains('offerInitiatorName ='));
      expect(
        at,
        isNot(-1),
        reason: 'в $chat нет строки с «offerInitiatorName =» — место '
            'переименовано либо снято, и проверки ниже сторожат пустоту (I13)',
      );
      final block = lines.skip(at).take(4).join('\n');

      expect(
        block,
        contains('_otherUserCached'),
        reason: 'имя предложившего снова берётся не у живого пользователя. '
            'Поле `name` прямого чата пишет создатель и не обновляет '
            '(chat_screen.dart:4418) — вторая сторона прочтёт оттуда СВОЁ имя.',
      );
      expect(
        block,
        isNot(contains('chatData')),
        reason: 'источник имени снова поле чата — см. довод выше',
      );
    });
