...
void main() {
  ...
  group('предложение работы создаётся одним кодом (пункт 6)', () {
    ...
    // --- ЧЕГО НЕ ЛОВИТ (I50) ---
    //
    // **1. ТРЁХ ОСТАВШИХСЯ ЧИТАТЕЛЕЙ ТОГО ЖЕ ПОЛЯ, и они названы поимённо,
    // потому что «починено» без доли врёт (I64): соблюдено 2 из 5.**
    // `chat_screen.dart` — подпись цитируемого в поле ввода и запасной путь
    // заголовка прямого чата (искать по `_replyingTo!.senderId == currentUid`
    // и по `displayName =`); `message_info_screen.dart` — «$otherName oxudu».
    // **Все три только рисуют, в данные не пишут** — это и есть довод, почему
    // они ждут, а эти два нет.
    //
    // 2. ЦЕЛОСТЬ КУСКА (N104). Срез по имени доказывает, что кусок ТОТ, и не
    ...
    //    выродилось.
    // 3. Что `_otherUserCached` заполнен верно — вопрос к `currentUserProvider`.
    // 4. Подпись СВОЕЙ стороны: она идёт от `FirebaseAuth`, подменить нечем.
    // 5. Третью ветвь — что в ГРУППЕ имени нет. Её держит `_isDirectChatCached`,
    //    и проверяется он ниже отдельной строкой, а не этим срезом.
    test('е: имя человека берётся у живого пользователя, оба места', () {
      const chat = 'lib/features/chat/screens/chat_screen.dart';
      final lines = readCode(chat).split('\n');

      // Два места, одна проверка. Длина среза — по телу каждого, не «с
      // запасом»: лишние строки затянули бы в кусок соседний код и сделали
      // бы запрет на `chatData` случайным.
      const places = <String, ({String anchor, int span, String what})>{
      ...
      for (final p in places.entries) {
        ...
        expect(
          ...
          1,
          reason: '$name: «$anchor» встречается не один раз — значит место '
              'раздвоилось, и источников имени снова несколько',
        );
        ...
        expect(
          at,
          isNot(-1),
          reason: '$name: в $chat нет строки с «$anchor» — место '
        ...
        // Соседка к срезу: пустой или выродившийся кусок дал бы «нет
        // chatData» даром, ничего не прочитав.
        expect(
          block.trim().length,
          greaterThan(40),
          reason: '$name: срез вышел пустым (${block.trim().length} знаков) — '
              'запрет ниже проверяет пустоту, а не код',
        );

        expect(
          block,
          contains('_otherUserCached'),
          reason: '$name: имя снова берётся не у живого пользователя. Поле '
              '`name` прямого чата пишет СОЗДАТЕЛЬ и не обновляет никогда — '
              'вторая сторона прочтёт оттуда СВОЁ имя, а в новых чатах '
              'пустоту. Цена: ${p.value.what}.',
        );
        expect(
          block,
          isNot(contains('chatData')),
          reason: '$name: источник имени снова поле чата — см. довод выше',
        );
      }
      ...
      final replyAt =
          lines.indexWhere((l) => l.contains('String _replySenderName('));
      expect(
        lines.skip(replyAt).take(6).join('\n'),
