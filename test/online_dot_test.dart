...
void main() {
  ...
  group('кружок онлайна: что он показывает', () {
    ...
    testWidgets('человека ещё нет — серый, а не пропавший кружок', (tester) async {
      // ПЯТЬ мест из двенадцати подают в кружок значение, которое МОЖЕТ
      // быть пустым. Пустота обязана быть законным входом, иначе у зовущего
      // снова заводится своё условие.
      //
      // ЧИСЛО СНЯТО ЗАМЕРОМ 19.09, А НЕ ПЕРЕЧИСЛЕНИЕМ (I57): поле `user` в
      // `online_dot.dart` временно объявлено непустым, `flutter analyze lib`
      // назвал отказы поимённо, поле возвращено. Так считает тот, кто считает
      // по определению (I13) — перечисление по памяти дало бы соседний ответ.
      //
      // ЗДЕСЬ СТОЯЛО «Четыре места из тринадцати», и четвёрка была неверна
      // ещё до этой работы: снятый 19.09 лист подавал непустое `m`, то есть
      // в этот счёт не входил вовсе. Знаменатель уехал с 13 на 12 по другой
      // причине — мест с кружком стало меньше.
      //
      // ОГОВОРКА, ЧТОБЫ ЧИСЛО НЕ ПРОЧЛИ ШИРЕ: считано по ТИПУ. «Сколько мест
      // рисуют кружок раньше, чем приедет живой документ» — соседний вопрос,
      // и ответ у него может быть другим: место вправе отсеять пустоту у
      // себя и подать непустое.
  ...
  group('кружок онлайна: вид берётся у зовущего', () {
    ...
    testWidgets('без просьбы — двенадцать и рамка под цвет kBg2', (tester) async {
      // Умолчание описывает БОЛЬШИНСТВО, а не «как получилось»: так стояло
      // в десяти местах из тринадцати на 18.09. С 19.09 мест двенадцать —
      // пересчитывается `grep -rn "OnlineDot(" lib`, и соседний вердикт
      // «ДОЛЯ ЧИСЛОМ» краснеет, если число разойдётся.
  ...
  group('сторож: кружок онлайна рисуется одним местом', () {
    ...
    test('ДОЛЯ ЧИСЛОМ: двенадцать мест зовут кружок', () {
      // I64 — проверяется не наличие правила, а КАЖДЫЙ, кто под него
      // подпадает. «Правило записано» соврать может; «двенадцать из
      // двенадцати» — нет.
      //
      // Завёл новый кружок — это число меняется здесь руками, и в том его
      // смысл: пересчёт обязателен, а не желателен.
      //
      // БЫЛО ТРИНАДЦАТЬ В ОДИННАДЦАТИ ФАЙЛАХ, СТАЛО ДВЕНАДЦАТЬ В ДЕСЯТИ —
      // 19.09, вместе со снятием `_ParticipantPickerDialog`
      // (`agreements_screen.dart`). Это не потеря кружка: два листа выбора
      // людей на вечер сведены в один, и кружок снятого листа был как раз из
      // тех, что ВРАЛИ — он кормился снимком Algolia и гас у всех через две
      // минуты после открытия (N248).
      //
      // ЧИСЛО ЗДЕСЬ И ПОЙМАЛО ЭТУ ПРАВКУ, ровно за тем и стоит: прицельный
      // прогон пяти наборов её не видел, а полный покраснел в тот же заход.
      //
      // ЧЕГО ЭТО ЧИСЛО НЕ ГОВОРИТ: сколько из двенадцати показывают ПРАВДУ.
      // На 19.09 врут три из двенадцати — лист добавления в группу, создание
      // группы и поиск людей в списке чатов, все трое кормятся Algolia
      // (N248, к починке в этой работе не бралась).
      ...
      expect(total, 12,
          reason: 'мест с кружком стало $total вместо двенадцати:\n'
