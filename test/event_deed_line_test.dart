...
void main() {
  ...
  group('тон — громко только про участника', () {
    ...
    test('ОТМЕНА СВОЕГО ВЕЧЕРА НЕ громкая, и это решение, а не пропуск', () {
      // Отмена — тоже потеря, но потеря ДРУГОГО: вечера, а не человека.
      // Покрась её кирпичным — тон перестанет что-либо означать, и глаз
      // привыкнет к нему ровно там, где он должен останавливать.
      expect(deed(kDeedOwnerCancelled)?.tone, DeedTone.plain);
      expect(deed(kDeedCancelConfirmed)?.tone, DeedTone.plain);
    });

    test('А ВОТ ИСЧЕЗНУВШАЯ РАБОТА — ГРОМКАЯ, СВОИМ ТОНОМ (N222, 11.09)', () {
      // ЗДЕСЬ СТОЯЛО `DeedTone.plain` У `workCancelled`, В ОДНОМ РЯДУ С ДВУМЯ
      // СОСЕДЯМИ ВЫШЕ. Изменено решением владельца 11.09 по снимку с трубки:
      // «должен быть красным, предупреждающим».
      //
      // ПОЧЕМУ ДОВОД СОСЕДА ВЫШЕ НЕ ОТМЕНЁН, А РАЗГРАНИЧЕН. Он верен для
      // отмены СВОЕГО вечера его же владельцем: тот сам нажал, новостью это
      // ему не является. `workCancelled` пишет СЕРВЕР приглашённому, у
      // которого чужая работа исчезла из-под уже данного согласия, — это
      // новость, требующая решения.
      //
      // И ТРЕТИЙ ТОН, А НЕ КИРПИЧНЫЙ: кирпичный означает «человека не
      // стало». Слить их значило бы сказать о работе то, что говорят о людях.
      expect(deed(kDeedWorkCancelled)?.tone, DeedTone.workGone);
    });
    ...
    // ГРАНИЦА, НАЗВАННАЯ ЧИСЛОМ: тонов три, и каждый занят ОДНИМ смыслом.
    // Без этого вердикта третий тон через месяц раздадут «похожим» случаям,
    // и он перестанет означать что-либо — ровно то, чем грозил довод у
    // `memberGone`.
    test('тонов три, и у каждого свой смысл — поимённо', () {
      expect(DeedTone.values.length, 3);
      expect(deed(kDeedLeft)?.tone, DeedTone.memberGone);
      expect(deed(kDeedWorkCancelled)?.tone, DeedTone.workGone);
      expect(deed(kDeedCreated)?.tone, DeedTone.plain);
    });
