...
void main() {
  ...
  group('тон — громко только про участника', () {
    ...
    test('ИСЧЕЗНУВШАЯ РАБОТА — ТОЖЕ ТИХАЯ: красный отдан состоянию', () {
      // ДНЁМ 11.09 ЗДЕСЬ СТОЯЛ ТРЕТИЙ ТОН (`workGone`, N222). Снят решением
      // владельца тем же вечером: «красный должен значить одно». Красным
      // теперь пишется СОСТОЯНИЕ приглашения — «Şübhə altında» строкой выше
      // (`invitationStateLabel`), а поступок под ним — подпись, «кто и что».
    ...
    // ГРАНИЦА, НАЗВАННАЯ ЧИСЛОМ: тонов два, и каждый занят ОДНИМ смыслом.
    // Третий уже заводился (11.09 днём) и снят тем же вечером; без этого
    // вердикта его заведут снова «похожему» случаю, и красных строк на
    // экране станет две — ровно то, от чего снимали.
    test('тонов два, и у каждого свой смысл — поимённо', () {
      expect(DeedTone.values.length, 2);
      expect(deed(kDeedLeft)?.tone, DeedTone.memberGone);
      expect(deed(kDeedWorkCancelled)?.tone, DeedTone.plain);
