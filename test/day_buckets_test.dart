...
void main() {
  ...
  group('дневной экран подключён и открывается первым (работа 6, вариант А)', () {
    ...
    test('день и месяц — ОДИН переключатель, и он в ШАПКЕ (12.08)', () {
      final screen =
          codeOf('lib/features/agreements/screens/agreements_screen.dart');
      // УСТРОЙСТВО СМЕНИЛОСЬ 12.08: трёх закладок в шапке («Müqavilələr»,
      // «Təqvim», «Tədbirlər») больше нет вовсе — их место занял сам
      // переключатель «Gün | Ay». Прежняя редакция этой проверки искала
      // `view: 'calendar'` канарейкой и покраснела в тот же заход, когда
      // закладки убрали. Она сработала как задумана: сказала не «дефект», а
      // «форма записи сменилась» — и это разные новости.
      //
      // КАНАРЕЙКА ТЕПЕРЬ ДРУГАЯ, той же формы: ищется заведомо существующий
      // литерал нового устройства.
      expect(
        screen.contains("seg('gun', 'Gün')"),
        isTrue,
        reason: 'в прочитанном экране нет даже переключателя «Gün» — форма '
            'записи сменилась или файл не прочитался. Проверка ниже тогда '
            'зелена не потому, что закладок нет.',
      );
      expect(
        screen.contains("view: 'day'"),
        isFalse,
        reason: 'день снова стал отдельной закладкой в шапке: день и месяц — '
            'один вопрос на разном расстоянии, им место в одном переключателе',
      );
      expect(
        screen.contains("_buildHeaderTab"),
        isFalse,
        reason: 'три закладки вернулись в шапку — решением 12.08 их место '
            'занял переключатель дня и месяца',
      );
      expect(
        RegExp(r"_calendarMode\s*=\s*'gun'").hasMatch(screen),
        isTrue,
        reason: 'календарь больше не открывается днём — человек попадает на '
            'сетку месяца вместо своего вечера',
      );
    });
    ...
    test('нажатие на день отвечает ПОД СЕТКОЙ, а не уводит на «Tədbirlər»', () {
      ...
      // КАНАРЕЙКА, и здесь она нужнее всех прочих: искомое утверждается
      // отсутствием НЕ ПО ФАЙЛУ, А ПО ВЫРЕЗАННОМУ ИЗ НЕГО КУСКУ (I31).
      // Кусок задан двумя именами-границами, и стоит телу обработчика
      // переехать между ними — `tap` станет коротким или пустым, а
      // «увода на другую закладку нет» останется зелёным навсегда.
      // Падение по `substring` от этого не спасает: оно ловит только
      // случай, когда границы пропали совсем.
      //
      // Ищется СРАВНЕНИЕ, а не голое имя: `contains('_selectedCalendarDay')`
      // доказывал бы упоминание, и его справедливо не пропускает сторож
      // сторожей (`guards_are_guards_test`) — имя есть в файле всегда.
      // Форма `a == b` — то самое лечение, которое он и предписывает.
      expect(
        tap.contains('_selectedCalendarDay == day'),
        isTrue,
        reason: 'вырезанный кусок (${tap.length} симв.) не содержит даже '
            'сравнения `_selectedCalendarDay == day` — значит это уже не '
            'тело _onDayTap. Проверка ниже смотрит не туда и зелена сама '
            'по себе.',
      );
      expect(
