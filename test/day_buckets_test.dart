...
void main() {
  ...
  group('дневной экран подключён и открывается первым (работа 6, вариант А)', () {
    ...
    test('день и месяц — ОДНА закладка, переключателем (не четвёртой)', () {
      final screen =
          codeOf('lib/features/agreements/screens/agreements_screen.dart');
      // КАНАРЕЙКА (I31): проверка ниже утверждает ОТСУТСТВИЕ, и ослепший
      // разбор подтверждает его молча — пустая строка не содержит
      // `view: 'day'` ровно так же, как правильный экран. Ищется ТЕМ ЖЕ
      // `contains` и ТОЙ ЖЕ формы литерал, только заведомо существующий:
      // закладки в шапке объявлены как `view: 'calendar'` и
      // `view: 'agreements'`.
      expect(
        screen.contains("view: 'calendar'"),
        isTrue,
        reason: 'в прочитанном экране нет даже `view: \'calendar\'` — форма '
            'записи закладок сменилась или файл не прочитался. Проверка '
            'ниже тогда зелена не потому, что дня в шапке нет.',
      );
      expect(
