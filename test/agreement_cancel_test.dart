...
void main() {
  ...
  group('имена поступков совпадают с правилами буквально', () {
    ...
    test('служба шлёт ровно эти имена и ничего иного', () {
      ...
      //
      // `'created'` УШЁЛ ОБРАТНО В СЛЕПУЮ ЗОНУ 17.09, И ЭТО НЕ ПОТЕРЯ ХОДА.
      // `createLineupInvitation` снят вместе с документами-приглашениями
      // (закон о договоре, `CLAUDE.md`): позванный — участник того же вечера.
      // Поступок `created` служба пишет по-прежнему — в `addPersonalEvent`, —
      // но там он собран условием, и разбор его не видит, ровно как не видел
      // до 09.09. **Перемер это и показал: разбор дал множество без
      // `created`, а не «`created` больше не пишется».**
      //
      // `'edited'` ПРИБАВИЛСЯ ТЕМ ЖЕ ХОДОМ — его пишет `callPeopleToEvent`,
      // и пишет ЛИТЕРАЛОМ нарочно (I31: вынос за константу ослепил бы этот
      // самый разбор). Правилам он ничего не стоит: ветвь владельца
      // перечисления ключей не имеет, а `namesCancelDeed()` запрещает ровно
      // четыре имени отмены, среди которых `edited` нет. Имя выбрано не по
      // вкусу: `planUpdatePushes` ветвится ПО ИМЕНИ, и восемь имён выходят
      // из неё раньше разбора состава — поставь любое из них, и позванный
      // не получит «Tədbirə əlavə olundunuz» вовсе.
      expect(
        names,
        {'edited', kRestoredDeed, ...kCancelDeeds},
