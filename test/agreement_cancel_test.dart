...
void main() {
  ...
  group('имена поступков совпадают с правилами буквально', () {
    ...
    test('служба шлёт ровно эти имена и ничего иного', () {
      ...
      // `'created'` ПРИБАВИЛСЯ 09.09, И ЭТО НЕ НОВОЕ ИМЯ В ПРОЕКТЕ, А
      // НОВОЕ ИМЯ ДЛЯ ЭТОГО РАЗБОРА. Поступок `created` пишет и старый
      // `addPersonalEvent`, с самого начала, — но там он собран условием
      // (`replacedEventId == null ? 'created' : 'replaced'`), а разбор
      // читает только литерал `'lastActionType': '...'`. То есть слепая
      // зона, названная тремя абзацами ниже вслух, до сегодня скрывала
      // имя, которое в проекте уже жило.
      //
      // Заметен он стал оттого, что `createLineupInvitation` (работа 7,
      // шаг 5) пишет то же имя ПРЯМО, без условия. Правилам это ничего не
      // стоит: у `allow create` перечисления ключей нет вовсе, и обе
      // записи — создание документа, а не правка.
      //
      // ЧТО ИЗ ЭТОГО СЛЕДУЕТ ДЛЯ ЧИТАТЕЛЯ: множество ниже — это имена,
      // которые разбор ВИДИТ, а не все, которые служба шлёт. `'replaced'`
      // здесь по-прежнему нет, и его отсутствие ничего не доказывает.
      expect(
        names,
        {'created', kRestoredDeed, ...kCancelDeeds},
