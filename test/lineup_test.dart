...
//      шаг 5 ставил зовущему четыре строки на одном вечере.
//
// ТРЕТЬИМ БЫЛ сторож двойного нажатия (`invitedUidsUnder`); снят 17.09 вместе
// с документами-приглашениями — разбор ниже, на месте его вердиктов.
//
// ВТОРОЙ ТОЖЕ НАЗНАЧЕН К СНЯТИЮ и держится до своего шага: прятать у зовущего
// стало нечего, детей нет. Сегодня он стережёт девять приглашений, оставшихся
// в проде до общего стирания (решение владельца 17.09 — стереть одним шагом,
// когда клиент готов).
//
...
void main() {
  ...
  // N97 — «ЗАНЯТОСТЬ ЗОВУЩЕГО В ЭТОТ ВЕЧЕР ЕСТЬ УСЛОВИЕ ЗАДАЧИ, А НЕ ПОМЕХА».
  //
  // ГДЕ ДЕФЕКТ БЫЛ ДОСТИЖИМ НА САМОМ ДЕЛЕ — не там, где его ждали. Находка
  // писалась 09.08 под замысел «приглашение идёт через машину предложения
  // работы», и лечение планировалось как «провести id в `excludeEventId`»
  // (шаг 7, `docs/plan.md`). Построено иначе: путь приглашения не показывает
  // ни занятости, ни конфликтов вовсе — упираться не во что.
  //
  // Достижим он был в ПРАВКЕ РОДИТЕЛЯ: форма исключает правимый вечер по
  // `excludeEventId`, а его дети — другие id при том же часе, и потому
  // попадали в конфликты. Зовущий, двигая время своего же вечера, получал
  // предупреждение про приглашения, которые сам и разослал.
  //
  // ЛЕЧЕНИЕ ОКАЗАЛОСЬ ДРУГИМ: схлопывание детей в `conflictEventsOnDay`
  // (N211), сделанное по совсем другому поводу. `excludeEventId` проводить
  // никуда не пришлось — он и так на месте у всех своих вызывающих.
  group('N97 — правка родителя не конфликтует со своими приглашениями', () {
    final parent = PersonalEvent.fromFirestore('p', {
      'ownerUid': 'rafael',
    ...
    List<PersonalEvent> kids(int n) => [
          for (var i = 0; i < n; i++)
            PersonalEvent.fromFirestore('k$i', {
              'ownerUid': 'rafael',
              'parentEventId': 'p',
              'date': '2026-09-16T18:00:00',
              'musicians': ['g$i'],
              'answers': {'g$i': kAnswerWaiting},
              'answersWrittenByOwner': true,
              'status': 'agreed',
            }),
        ];

    test('двигая время своего вечера, зовущий НЕ упирается в своих же', () {
      final conflicts = conflictEventsOnDay(
        DateTime(2026, 9, 16, 18),
        [parent, ...kids(3)],
        currentUid: 'rafael',
        // Так делает форма правки: `_excludeEventId => existingEvent?.id`.
        excludeEventId: 'p',
      );
      expect(conflicts, isEmpty,
          reason: 'зовущему показали конфликт с приглашениями, которые он '
              'сам и разослал (N97)');
    });

    test('КАНАРЕЙКА: ЧУЖОЙ вечер в тот же час по-прежнему мешает', () {
      // I14, вторая половина: назвать, каким был бы вывод на исправном. Без
      // этой соседки правило, выключившее конфликты вовсе, выглядело бы
      // починкой N97 — а это худшая из возможных поломок на этом экране.
      //
      // **ПРОВЕРЯЕТСЯ НАЛИЧИЕ ЧУЖОГО, А НЕ РАВЕНСТВО СПИСКА, И ЭТО ПРАВКА ПО
      // ЖИВОМУ ПРОМАХУ 09.09.** Здесь стояло `expect(map(id), ['чужой'])` —
      // точное равенство, — и при снятом схлопывании канарейка падала ВМЕСТЕ
      // с основным вердиктом. То есть она была его копией, а не соседкой:
      // соседка обязана падать на ПРОТИВОПОЛОЖНОЙ поломке — когда
      // схлопывание съело лишнее, — и молчать на той, которую ловит сосед.
      //
      // Промах вскрылся порчей: сказал «упадут два», упало три. Считать надо
      // было `expect` внутри, а не заголовок (N207).
      final foreign = PersonalEvent.fromFirestore('чужой', {
        'ownerUid': 'кто-то',
        'date': '2026-09-16T18:00:00',
        'musicians': const ['rafael'],
        'answers': const {'rafael': kAnswerGoing},
        'answersWrittenByOwner': true,
        'status': 'agreed',
      });
      final conflicts = conflictEventsOnDay(
        DateTime(2026, 9, 16, 18),
        [parent, ...kids(3), foreign],
        currentUid: 'rafael',
        excludeEventId: 'p',
      );
      expect(conflicts.map((e) => e.id), contains('чужой'),
          reason: 'настоящий конфликт пропал вместе с повторами');
    });
  });

  // ЗДЕСЬ СТОЯЛ «СТОРОЖ ДВОЙНОГО НАЖАТИЯ» — ЧЕТЫРЕ ВЕРДИКТА НА
  // `invitedUidsUnder`. Снят 17.09 вместе с самой функцией.
  //
  // ЧТО ОН СТЕРЁГ И ПОЧЕМУ ЭТОГО БОЛЬШЕ НЕТ. Он отвечал на вопрос «кого уже
  // позвали под этот вечер», обходя ЧУЖИЕ документы по `parentEventId`: без
  // него повторное нажатие «Öz adamlarımı çağır» заводило человеку второе
  // приглашение, а лишние документы необратимы в данных. Позванный теперь —
  // участник ТОГО ЖЕ вечера (закон о договоре, `CLAUDE.md`), и «уже позван»
  // читается прямо из `musicians`: двойник в списке невозможен по устройству,
  // а не по стражe.
  //
  // ЧТО ИЗ ЧЕТЫРЁХ ВЕРДИКТОВ ПЕРЕЖИЛО СНЯТИЕ, названо поимённо, чтобы следующий
  // не счёл их потерянными:
  //   • «позванные названы поимённо» и «у вечера без приглашений пусто» —
  //     стали свойством состава, и их держит `lineup_rows_test.dart`;
  //   • «дети ЧУЖОГО родителя сюда не попадают» — вопроса нет: чужих
  //     документов правило больше не читает вовсе;
  //   • «ОТМЕНЁННОЕ ПРИГЛАШЕНИЕ СЧИТАЕТСЯ ПОЗВАННЫМ» — решение сохранено и
  //     выражено иначе: отменить приглашение по одному человеку нельзя
  //     (12.09), а снятие теперь и есть уход из состава.
