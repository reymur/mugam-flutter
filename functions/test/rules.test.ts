...
// ---------------------------------------------------------------------
// personalEvents — ВЫХОД ПРАВКОЙ СОСТАВА СНЯТ (N121, шаг 3, 30.08)
//
// Ход был заведён 03.08 («своё удаляется у всех, чужое — только у меня»)
// и снят вместе с правилом `leavesEvent()`: с 29.08 уход — это ответ
// человека (`answers.<uid> = 'left'`), а не правка состава.
//
// ПАРА ПРОВЕРОК, И ОНА ЗДЕСЬ ГЛАВНАЯ, а не одна половина из двух.
//
// Отрицание в одиночку доказывает не то, что думают: `assertFails` зеленеет
// и когда правило сняли верно, и когда сеанс не тот, вечер не тот, участник
// не участник, а эмулятор поднял чужой файл правил. Все эти отказы выглядят
// одинаково (I14: назвать, каким был бы вывод при поломке — и он совпадает
// с наблюдаемым).
//
// Поэтому рядом стоит СОСЕДНЯЯ дорога того же человека в том же вечере тем
// же сеансом: ответ за себя. Она обязана ПУСКАТЬ. Пропусти эмулятор мимо
// себя весь блок правил — покраснеет она, а не отрицание. Тот же приём, что
// с `dates.size()`: снятое больше не пускает, соседнее по-прежнему пускает.
// ---------------------------------------------------------------------

test("СНЯТО: участник НЕ может вычеркнуть себя из чужого мероприятия", async () => {
  // Ровно тот ход, который правило `leavesEvent()` разрешало до 30.08:
  // состав минус сам просящий, своя подпись, имя поступка `left`. Теперь
  // его не берёт ни одна ветвь `allow update`.
  const contactDb = testEnv.authenticatedContext(CONTACT).firestore();
  await assertFails(
    updateDoc(doc(contactDb, `personalEvents/${EVENT}`), {
      musicians: [OWNER],
...
test("СОСЕДНЕЕ ЦЕЛО: тот же участник МОЖЕТ выйти ответом за себя", async () => {
  // Вторая половина пары, и она же канарейка к первой (I31): утверждает
  // НАЛИЧИЕ, значит сама себе сторож. Тот же `CONTACT`, тот же `EVENT`, тот
  // же сеанс — расходится только дорога.
  //
  // Прав этим не прибавилось ни на йоту: `answersForSelf()` выложен 12.08 и
  // значение `'left'` разрешал уже тогда. Снятие `leavesEvent()` — сужение,
  // и эта проверка показывает, ЧТО именно осталось после сужения.
  const contactDb = testEnv.authenticatedContext(CONTACT).firestore();
  await assertSucceeds(
    updateDoc(doc(contactDb, `personalEvents/${EVENT}`), {
      [`answers.${CONTACT}`]: "left",
    }),
  );
});
...
test("выход: участник НЕ может под видом выхода изменить дату", async () => {
  // ЧЕТЫРЕ ОТРИЦАНИЯ НИЖЕ ПЕРЕЖИЛИ САМО ПРАВИЛО, И ЭТО НАРОЧНО. Они были
  // границами живого хода — «состав и только состав», «своего и только
  // своего», — а стали сторожами СУЖЕНИЯ: после 30.08 они обязаны падать
  // ВСЕГДА, каким бы способом кто-нибудь ни завёл участнику дорогу к
  // `musicians` заново. Держать их дешевле, чем однажды открыть эту дверь
  // второй раз и не заметить.
  //
  // Слепыми они при этом не станут: соседняя половина пары выше
  // (`answersForSelf`) утверждает наличие и краснеет, если отказывать
  // начнёт весь файл целиком.
  const contactDb = testEnv.authenticatedContext(CONTACT).firestore();
  await assertFails(
    updateDoc(doc(contactDb, `personalEvents/${EVENT}`), {
      musicians: [OWNER],
