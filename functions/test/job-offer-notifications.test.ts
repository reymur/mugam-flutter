...
describe("что уходит второй стороне", () => {
  ...
  // ИМЯ ПОПРАВЛЕНО 30.08, И ПОВЕДЕНИЕ ЗДЕСЬ НИ ПРИ ЧЁМ.
  //
  // Стояло «НЕ уходит: согласие уже покрыто onChatUpdated». Для СТАРОЙ
  // схемы, которую этот набор и проверяет, это по-прежнему правда:
  // `recipientAgreed` на документе чата пишет только старый ход, и
  // `onChatUpdated` шлёт по нему «İş təklifi qəbul edildi».
  //
  // Неправдой стало прочтение имени ШИРЕ набора — «согласие покрыто», без
  // оговорки, ЧЬЁ. Новая схема (подколлекция `offers`) `recipientAgreed` не
  // пишет вовсе, и её принятие покрыто другим — `onOfferMoved` (N130,
  // шаг 4). Оставь имя как было — и следующий, увидев его, решит, что
  // согласие покрыто ВЕЗДЕ.
  it("НЕ уходит: согласие СТАРОЙ схемы покрыто onChatUpdated", () => {
