...
/// ЧЕГО ТРИ ОТРИЦАТЕЛЬНЫХ ВЕРДИКТА НИЖЕ НЕ ДОКАЗЫВАЮТ — ДОЛГ N239 (16.09).
///
/// Все три устроены одинаково: рядом с молчаливым вечером заводится
/// заведомо шумный, ждут ЕГО, и после этого смотрят на молчуна. Приём
/// честнее таймера — тот зелен и когда триггер верно промолчал, и когда он
/// не запускался вовсе, — но **доказывает он приход СОСЕДА, а не молчание
/// МОЛЧУНА**: порядок между разными документами очередью не гарантирован и
/// под нагрузкой инвертируется.
///
/// «Рассылки не было» — утверждение об отсутствии, растянутое во времени: в
/// какую точку ни посмотри, оно говорит только про эту точку. Наблюдением
/// оно не устанавливается по определению.
///
/// **ЧТО НУЖНО, ЧТОБЫ ДОКАЗЫВАЛО:** отметка «обработчик отработал на этом
/// вечере и решил не слать», которую сервер кладёт ВСЕГДА, включая
/// молчание. Тогда ждут её по самому молчуну. Это правка в
/// `functions/src`, а не здесь, и потому решение владельца.
///
/// **Уникальные имена тут НЕ нужны, и это проверено чтением:** `notified`
/// ключуется по id вечера, а он свой у каждого вердикта; общий здесь только
/// документ владельца, и `makeEvent` пишет в него `arrayUnion`, а не `set`.
/// Чужой остаток под запрос не попадает.
///
/// **Ни один из трёх 16.09 не падал.** Записано как долг по чтению кода, а
/// не по воспроизведённому отказу; **N77 этим не закрывается.**
