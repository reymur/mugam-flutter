...
// КАНАРЕЙКА, И ЗДЕСЬ ОНА ПРОВЕРЯЕТ РОВНО ТО, РАДИ ЧЕГО ВЫКЛАДКА ЗОВЁТСЯ
// ИНЕРТНОЙ: старый путь — вычёркивание из `musicians` с `lastActionType:
// 'left'` — работает как работал. Сегодня в проде идёт только он: записей
// `answers.* == 'left'` там ноль при 28 вечерах с полем `answers` (замер
// 27.08). Съешь новая ветвь старую, покраснеет здесь.
test("КАНАРЕЙКА: уход по СТАРОЙ схеме по-прежнему даёт рассылку", async () => {
  const ref = await makeEvent();
  await ref.update({
    musicians: [OWNER],
    lastActionBy: GUEST,
