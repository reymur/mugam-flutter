...
describe("шаг: состояние вечера ставит владелец", () => {
  ...
  it("владелец отменяет вечер С ДОГОВОРЁННОСТЬЮ так же, как любой", async () => {
    // ВЕРДИКТ ПЕРЕВЁРНУТ 12.09 (N225), А НЕ СНЯТ. Здесь стояло «НЕЛЬЗЯ
    // отменить вечер С ДОГОВОРЁННОСТЬЮ в одиночку» с доводом «расторгнуть
    // договорённость требует согласия второй стороны».
    //
    // Довод снят решением владельца по записи 10.08 «договора как отдельной
    // вещи больше нет»: вечер один, `isAgree` лишь помечает, откуда он
    // взялся. Путь согласия снят с экрана 13.08 и вызывающих не имеет, а
    // кнопка «Ləğv et» у владельца стоит без условия по `isAgree` — то есть
    // прежнее правило отказывало живой кнопке.
