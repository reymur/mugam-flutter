...
describe("состав участников", () => {
  ...
  // СНЯТО 30.08 (N121, шаг 3). Здесь стоял положительный случай «вышел сам —
  // узнаёт владелец»: правка `musicians` с `lastActionType: 'left'` давала
  // `pushLeft` владельцу. Ветвь снята вместе с правилом `leavesEvent()`,
  // и на её место встала проверка НАОБОРОТ.
  it("СНЯТО: старый путь (правка musicians + 'left') об уходе не пишет", () => {
    ...
    // Ни одного «İştirakçı ayrıldı»: имя пришлось бы взять из `lastActionBy`,
    // а он после снятия правила может принадлежать только владельцу или
    // чужому писателю (I49) — и владелец прочитал бы, что вышел он сам.
    assert.equal(pushes.some((p) => p.title === "İştirakçı ayrıldı"), false);
    // И «вас исключили» тоже нет: автор записи снимается из адресатов.
    assert.equal(pushes.some((p) => p.uid === GUEST), false);
  });

  it("вышедший сам не получает «вас исключили»", () => {
    // Пережило снятие ветви и проверяет теперь ДРУГОЕ, поэтому оставлено, а
    // не выброшено: раньше сюда не доходило вовсе — ранний `return` старой
    // ветви уводил разбор, — а теперь запись падает в общий обход
    // `diff.removed`, и молчание держится на одной строке `uid === actor`.
    const pushes = plan({
      eventId: "e1",
      before: ev(),
...
describe("участник вышел — НОВАЯ схема, переход answers → left (N121)", () => {
  ...
  //
  // Здесь стояла проверка «обычный уход по СТАРОЙ схеме по-прежнему даёт»:
  // она утверждала НАЛИЧИЕ старой ветви и падала, если бы новая её съела.
  // Старой ветви больше нет, и утверждать про неё наличие нечего — такая
  // канарейка обязана уйти, а не быть вывернутой в отрицание: отрицание
  // про несуществующее и есть та самая проверка, которая не может
  // провалиться (I9). Отрицательный случай про старый путь стоит ровно
  // ОДИН и в блоке про удаление выше, где ему место.
  //
  // ЧЕМ ЗАКРЫТЫ ПЯТЬ ОТРИЦАНИЙ ЭТОГО БЛОКА ПОСЛЕ ЕЁ УХОДА — назвать
  // обязательно, иначе они останутся без соседки (I13): тремя
  // положительными случаями В ЭТОМ ЖЕ БЛОКЕ — «переход в 'left' даёт
  // уведомление», «имя берётся от uid перехода» и «имени нет в карте —
  // запасное слово». Все три утверждают наличие и краснеют, если ветвь
  // перестанет считаться вовсе.
...
describe("отмена личного вечера владельцем (N213)", () => {
  ...
  it("workCancelled остаётся без ветви, и это НАРОЧНО", () => {
    // ИМЯ ЖИВЁТ В ДАННЫХ ПРОДА, А ПИСАТЕЛЯ У НЕГО НЕТ С 12.09:
    // `unsettleChildrenOnParentCancel` снят вместе со всей автоматикой с
    // детьми. Ветвь здесь не заводится — письмо по такому документу было бы
    // рассказом о действии, которого никто не совершал.
    //
    // ВЕРДИКТ ОСТАВЛЕН, А НЕ СНЯТ: документы с этим именем в проде лежат, и
    // первый же, кто заведёт ветвь «чтобы не молчало», получит письмо о
    // снятой работе. Здесь стояла граница между починкой N213 и шагом 8 —
    // шага 8 больше нет, а граница нужна прежняя.
    const pushes = plan({
      eventId: "e1",
      before,
      after: ev({
...
// ---------------------------------------------------------------------------
// ПЕРЕНОС ПРАВКИ РОДИТЕЛЯ В ПРИГЛАШЕНИЯ — работа 7, шаг 6, 09.09
// ---------------------------------------------------------------------------
{
  ...
  const at = (over: Partial<EventSnapshot>): EventSnapshot => ({ ...base, ...over });

  // Сдвинули дату — переносим её, и только её.
  {
    const patch = childPatchForParentEdit(base, at({ date: "2026-09-17T21:00:00" }));
    assert.deepEqual(patch, { date: "2026-09-17T21:00:00" });
  }

  // Место и заметки — тем же правилом.
  assert.deepEqual(
    childPatchForParentEdit(base, at({ location: "Baglar" })),
    { location: "Baglar" },
  );
  assert.deepEqual(
    childPatchForParentEdit(base, at({ notes: "Ag koynek" })),
    { notes: "Ag koynek" },
  );

  // Три сразу — три поля, а не «первое найденное».
  assert.deepEqual(
    childPatchForParentEdit(
      base,
      at({ date: "2026-09-17T21:00:00", location: "Baglar", notes: "Ag koynek" }),
    ),
    { date: "2026-09-17T21:00:00", location: "Baglar", notes: "Ag koynek" },
  );

  // ПРИЗНАК ПЕРЕХОДА, А НЕ СОСТОЯНИЯ: ничего не менялось — переносить нечего.
  // Без этого перенос шёл бы на КАЖДОЙ записи в родителя, включая запись
  // шаблона состава, и каждая поднимала бы N записей и N уведомлений.
  assert.equal(childPatchForParentEdit(base, at({})), null);

  // Изменилось ПОСТОРОННЕЕ — тоже ничего. Здесь именно шаблон состава:
  // `_callMyPeople` пишет его в родителя одной операцией сразу после
  // создания детей, и перенос на нём означал бы обход сразу вслед за
  // созданием, впустую.
  assert.equal(
    childPatchForParentEdit(base, at({ lastActionType: "edited" })),
    null,
  );

  // СОСТАВ НЕ ПЕРЕНОСИТСЯ. У ребёнка он свой — ровно один приглашённый;
  // перенеси его, и приглашённый увидел бы в своём вечере чужие фамилии.
  assert.equal(
    childPatchForParentEdit(base, at({ musicians: ["a", "b", "c"] })),
    null,
  );

  // СТАТУС НЕ ПЕРЕНОСИТСЯ, И ЭТО НЕ ЗАБЫВЧИВОСТЬ. Отмена родителя ведёт
  // детей ИНАЧЕ — под вопрос с поводом workCancelled (шаг 8, решение
  // владельца 08.09). Перенеси его здесь, и дети отменились бы за людей,
  // которые ничего не отменяли, а шаг 8 стал бы недостижим.
  assert.equal(
    childPatchForParentEdit(base, at({ status: "cancelled" })),
    null,
  );

  // РЕБЁНОК ДЕТЕЙ НЕ ИМЕЕТ — обход по нему не запускается вовсе. Само по
  // себе оно завершилось бы и так (запрос пуст), но тогда каждая правка
  // приглашения стоила бы лишний запрос, а завершение держалось бы на
  // совпадении.
  assert.equal(
    childPatchForParentEdit(
      at({ parentEventId: "p" }),
      at({ parentEventId: "p", date: "2026-09-17T21:00:00" }),
    ),
    null,
  );

  // Потолок назван числом и заметно велик против наблюдаемого: самый большой
  // состав за историю прода — 10 человек (замер 10.08).
  assert.ok(kMaxLineupChildren >= 50, "потолок опустился ниже разумного");
}

// ---------------------------------------------------------------------------
// ПРОВОДКА ПЕРЕНОСА — сторож по исходнику, работа 7, шаг 6, 09.09
// ---------------------------------------------------------------------------
// ЗАЧЕМ ПО ИСХОДНИКУ, А НЕ ПОВЕДЕНИЕМ. Оба вердикта ниже про то, чего чистое
// правило не видит: КАК триггер сложен. Ошибка ровно в одном из этих двух
// мест стоит либо непереносa полей, либо неотправленных уведомлений — и
// снаружи, без эмулятора, эти два исхода неотличимы от «всё хорошо».
{
  const src = readFileSync(`${__dirname}/../src/index.ts`, "utf8");

  // КАНАРЕЙКА К ВЫРЕЗКЕ (I13, I31): без неё ноль совпадений ниже читался бы
  // как порядок, а означал бы «триггер переименовали».
  const start = src.indexOf("export const propagateParentEdits");
  assert.ok(start !== -1, "триггер переноса исчез — сторож смотрит в пустоту");
  const end = src.indexOf("export const markEventUnsettled", start);
  assert.ok(end !== -1, "вторая граница вырезки потеряна");
  const body = src.slice(start, end);
  assert.ok(
    body.includes("childPatchForParentEdit("),
    "канарейка: тело триггера не читается — вердикты ниже зелены даром",
  );

  // 1. ЗАМОК ПЕРЕНОСА — НЕ ТОТ ЖЕ, ЧТО У УВЕДОМЛЕНИЙ.
  //
  // `event.id` у всех триггеров одной записи ОБЩИЙ. Возьми перенос
  // `claimNotificationOnce` напрямую — и две функции подрались бы за один
  // `create`: победитель сделал бы своё, проигравший молча пропустил бы
  // свою работу. Либо не ушли бы уведомления, либо не переехали бы поля, и
  // вслепую эти исходы неразличимы.
  assert.ok(
    body.includes("claimPropagateOnce(event.id)"),
    "перенос перестал брать свой замок",
  );
  assert.ok(
    !body.includes("claimNotificationOnce("),
    "перенос взял ЗАМОК УВЕДОМЛЕНИЙ: `event.id` у триггеров одной записи " +
      "общий, и одна из двух функций молча пропустит свою работу",
  );

  // 2. ПЕРЕНОС НЕ ШЛЁТ УВЕДОМЛЕНИЙ САМ.
  //
  // Их шлёт `onPersonalEventUpdated`, сработавший НА РЕБЁНКЕ, и потому
  // несёт id ребёнка — то есть ведёт туда, что приглашённый вправе открыть.
  // Появись здесь своя рассылка, она понесла бы id РОДИТЕЛЯ, которого
  // приглашённый прочитать не может: `allow read` пускает владельца и
  // `musicians`, а его там нет. Уведомление, ведущее в отказ по правам,
  // хуже уведомления без перехода.
  assert.ok(
    !body.includes("sendEventPushes(") && !body.includes("sendPushToUid("),
    "перенос завёл собственную рассылку: она понесёт id РОДИТЕЛЯ, которого " +
      "приглашённый читать не вправе, и нажатие откроет отказ по правам",
  );

  // 3. ПОТОЛОК ПРОВЕДЁН И УСЕЧЕНИЕ НЕ МОЛЧИТ.
  assert.ok(
