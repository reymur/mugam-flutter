...
async function main(): Promise<void> {
  ...
  if (totalSinceOutage === 0) {
    // ЭТО ПЕЧАТАЕТСЯ ТОЛЬКО ПРИ НУЛЕ И ИМЕННО ПОЭТОМУ. Ноль расхождений
    // при нуле сообщений — не «обошлось», а «писать ещё не начинали», и
    // читается он как хорошая новость ровно так же (I14). Тот, кто
    // запустит перепись ПОСЛЕ оживления, увидит тот же ноль и решит, что
    // простой ничего не стоил, — а он к тому времени будет означать уже
    // другое.
    console.log("");
    console.log("0. НОЛЬ ЗДЕСЬ — «ПИСАТЬ ЕЩЁ НЕ НАЧИНАЛИ», А НЕ «ОБОШЛОСЬ».");
    console.log("   Сообщений с начала простоя — ноль, значит счётчику нечего");
    console.log("   было пропускать: расхождения нет не потому, что сервер");
    console.log("   успевал, а потому, что событий не было вовсе.");
    console.log("   ПЕРВОЕ ЖЕ сообщение, написанное при неработающих функциях,");
    console.log("   сделает этот ноль ненулевым НАВСЕГДА: onNewMessage не");
    console.log("   прибавит единицу, а пересчёта в проекте нет — ни открытие");
    console.log("   чата, ни новые сообщения отставания не сводят.");
    console.log("   Сравнивать надо не с нулём, а со столбцом «с простоя»:");
    console.log("   пока он нулевой, перепись говорит только о тишине.");
  }
  console.log("");
