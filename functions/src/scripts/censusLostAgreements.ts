...
//
// ═══════════════════════════════════════════════════════════════════════
// ЧИТАТЬ ДО ТОГО, КАК ПОВЕРИТЬ НУЛЮ. Два свойства этой сверки, из-за
// которых её ноль означает МЕНЬШЕ, чем выглядит. Оба печатаются и в
// выводе — потому что читают выхлоп, а не исходник.
//
// 1. НОЛЬ — ЭТО «НА ЭТУ МИНУТУ», А НЕ «НЕ ПОТЕРЯЛОСЬ».
//    Сверка видит пропажу, только пока на чате ещё стоит
//    `recipientAgreed: true`. А `setJobOffer` — новое предложение в том
//    же чате — пишет `recipientAgreed: false`, и след потерянного
//    договора исчезает НАВСЕГДА: ни поля, ни отметки, ни следа того, что
//    согласие вообще было.
//    **Поэтому снятие через неделю даст ноль и по чатам, где потеря
//    была.** Ноль этой сверки стареет; чем позже он снят, тем больше он
//    мог не досчитать. Отсюда правило снимать счёт ДО восстановления
//    оплаты и сравнивать с ним потом, а не мерить заново.
//
// 2. ПАРА ВЫРОЖДАЕТСЯ В ОДИН ЧАТ НА СТАРЫХ ДОГОВОРАХ.
//    Замер 09.08: `jobOfferAt` пуст у **71 договора из 72** — поле
//    пишется только с 04.08 (N29). Там, где отметки раунда нет с обеих
//    сторон, `sameRound(null, null)` даёт совпадение, и сверка
//    вырождается в «есть ли в этом чате хоть какой-нибудь договор». То
//    есть на старых данных она ГРУБЕЕ, чем выглядит, и второй потерянный
//    раунд в чате с одним уцелевшим договором она не заметит. Число
//    пустых отметок печатается ниже — если оно велико, точность сверки
//    надо читать по нему, а не по её виду.
// ═══════════════════════════════════════════════════════════════════════
//
...
async function main(): Promise<void> {
  ...
  // ЭТО ПЕЧАТАЕТСЯ ВСЕГДА, И ОСОБЕННО ВАЖНО ПРИ НУЛЕ. Читают вывод, а не
  // исходник; ноль без этих двух абзацев будет прочитан как «ничего не
  // потерялось» — а он значит другое.
  const emptyRounds = eventFormTally.get("null") ?? 0;
  console.log("");
  console.log("═══ ЧТО ЭТОТ СЧЁТ НЕ ЗНАЧИТ — читать до того, как поверить нулю ═══");
  console.log("");
  console.log("1. Ноль — это «НА ЭТУ МИНУТУ», а не «не потерялось».");
  console.log("   Пропажа видна, только пока на чате стоит recipientAgreed: true.");
  console.log("   Новое предложение в том же чате (setJobOffer) пишет false — и");
  console.log("   след потерянного договора исчезает НАВСЕГДА. Снятие через неделю");
  console.log("   даст ноль и по чатам, где потеря была. Этот ноль стареет:");
  console.log("   сравнивать надо со снимком, сделанным раньше, а не мерить заново.");
  console.log("");
  console.log(`2. Пара «чат + раунд» вырождается в один чат: jobOfferAt пуст у`);
  console.log(`   ${emptyRounds} договоров из ${eventsSnap.size}. Где отметки нет с обеих сторон,`);
  console.log("   сверка спрашивает лишь «есть ли в этом чате хоть какой-то договор»");
  console.log("   и второго потерянного раунда в таком чате НЕ ЗАМЕТИТ.");
  console.log("");
  console.log("3. Договор, созданный и потом УДАЛЁННЫЙ владельцем, здесь неотличим");
  console.log("   от несозданного. Отметка согласия идёт по часам устройства.");
}
