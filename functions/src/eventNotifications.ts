...
/**
 * КОГО СПРОСИЛИ ЗАНОВО — переход ключа в `waiting` у того, кто УЖЕ был в
 * составе (17.09, решение владельца «зов вышедшего работает»).
 *
 * **ЗЕРКАЛО `leftViaAnswers`, И ПРИЗНАК ТОТ ЖЕ — ПЕРЕХОД, А НЕ СОСТОЯНИЕ.**
 * Имя поступка сюда не годится: `lastActionType` у зова — `edited`, тот же,
 * что у обычной правки вечера, и различить их по рассказу нельзя. Подделать
 * переход в карте ответов нельзя вовсе — он и есть поступок (N243, I54).
 *
 * **ТРИ УСЛОВИЯ, И ТРЕТЬЕ НЕСУЩЕЕ.**
 *
 *   `after[uid] === "waiting"`  — вопрос перед человеком теперь стоит;
 *   `before[uid] !== "waiting"` — ПЕРЕХОД: он стоит не со вчера. Без этого
 *                                 ветвь стреляла бы на каждой следующей
 *                                 записи в документ — ответ соседа, правка
 *                                 места, серверный `update`, — и человек
 *                                 получал бы «вас добавили» без конца;
 *   `uid` был в `before.musicians` — ОН НЕ НОВЫЙ. Без этого условия каждый
 *                                 обычный зов слал бы новому человеку ДВА
 *                                 письма: одно отсюда, другое от `diff.added`
 *                                 ниже по `planUpdatePushes`.
 *
 * **ЛОВИТ ДВА СЛУЧАЯ, И ЭТО НЕ СЛИЯНИЕ ДВУХ НЕЗНАНИЙ (I47):** вышедший
 * (`left` → `waiting`) и тот, кого не спрашивали (ключа не было → `waiting`).
 * В данных они различны и различными остаются; одинаков ход — человека
 * спросили, и он обязан об этом узнать.
 *
 * **ПОРЯДОК В ОТВЕТЕ — ПО uid**, как у соседа: список идёт в письма и в
 * уборку, и сравнивать его в тестах надо с чем-то устойчивым.
 */
export function askedAgainViaAnswers(
  before: EventSnapshot,
  after: EventSnapshot,
): string[] {
  const b = before.answers ?? {};
  const a = after.answers ?? {};
  const wasInParty = new Set(before.musicians ?? []);
  return Object.keys(a)
    .filter((uid) =>
      a[uid] === "waiting" && b[uid] !== "waiting" && wasInParty.has(uid))
    .sort();
}
...
/**
 * «Вечер под вопросом» — ОДНО сообщение на ТРИ дороги в это состояние.
 *
 * Дороги: ушёл участник (`markEventUnsettled`, писателя у неё нет — уход
 * помечается на человеке), **владелец сам усомнился (`ownerDoubt`)** и
 * суточное напоминание соседом ниже.
 *
 * **ЧЕТВЁРТОЙ ДОРОГИ БОЛЬШЕ НЕТ: 12.09 снят `unsettleChildrenOnParentCancel`**
 * — отмена родителя детей не трогает. Выбранное владельцем состояние пишет
 * клиент разом родителю и детям, и каждый документ шлёт своё письмо этой же
 * функцией, то есть текст у состава и у приглашённых один.
 *
...
export function planUpdatePushes(input: PlanUpdateInput): EventPush[] {
  ...
  switch (after.lastActionType) {
    ...
    //
    // **ЭТА ЖЕ ВЕТВЬ ИЗВЕЩАЕТ И ПРИГЛАШЁННЫХ — с 12.09.** Клиент пишет
    // отмену разом родителю и детям, с тем же именем поступка; на каждом
    // ребёнке поднимается свой `onPersonalEventUpdated` и приходит сюда.
    // Приглашённый получает ОДНО письмо — своё, про свой документ, — и
  ...
  // СПРОШЕННЫЕ ЗАНОВО — ПИСЬМО ТЕМ ЖЕ ТЕКСТОМ, ЧТО И ДОБАВЛЕННЫМ (17.09).
  //
  // ПОЧЕМУ ЗДЕСЬ, А НЕ РАНЬШЕ С РАННИМ ВЫХОДОМ, как у `leftViaAnswers`. Одна
  // запись зова может и добавить нового, и вернуть вышедшего: первый получает
  // письмо ниже по `diff.added`, второй — здесь. Выйди мы отсюда сразу, и
  // новый остался бы без письма.
  //
  // ПОЧЕМУ ТОТ ЖЕ `pushAdded`, А НЕ СВОЙ ТЕКСТ. Для человека случилось ровно
  // то, что там написано: его добавили в вечер. Второй текст про то же
  // заставил бы его различать «добавили» и «добавили снова» — различие есть у
  // нас в данных, а у него в жизни его нет.
  //
  // ОТМЕТКА `touchedByActor` ОБЯЗАТЕЛЬНА. Без неё запись, которая и позвала
  // заново, и сдвинула дату, послала бы человеку два письма подряд — «вас
  // добавили» и «дата изменилась».
  for (const uid of askedAgainViaAnswers(before, after)) {
