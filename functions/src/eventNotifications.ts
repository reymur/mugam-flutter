...
export function planUpdatePushes(input: PlanUpdateInput): EventPush[] {
  ...
  // автора.
  //
  // --- ПОЧЕМУ ПЕРЕД СТАРОЙ ВЕТВЬЮ ---
  //
  // Пересечься они не могут: старый уход правит `musicians` и
  // `lastActionType` и не трогает `answers` (`leavesEvent()`, три ключа),
  // новый правит только `answers` (`answersForSelf()`, один ключ). Обе
  // `hasOnly`, значит одной записью оба хода не пройдут. Порядок всё равно
  // задан, а не оставлен на волю случая: сойдись они однажды от чужого
  // писателя (Admin SDK, консоль — I49), владелец получит ОДНО
  // уведомление, а не два об одном уходе.
  ...
  // Участник вышел сам — СТАРАЯ СХЕМА, через `musicians`. Уведомляется
  // владелец, и только он.
  //
  // НЕ СНИМАЕТСЯ ВМЕСТЕ С ПОЯВЛЕНИЕМ НОВОЙ, и это условие шага 1: на
  // трубках стоят сборки, зовущие старый путь, и снятие этой ветви отняло
  // бы у них единственный способ сообщить владельцу об уходе. Снимать её —
  // не раньше, чем таких сборок не останется (шаг 3, вместе с мёртвым
  // `leavesEvent()` в правилах).
