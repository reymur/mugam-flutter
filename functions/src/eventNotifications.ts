...
export type EventActionType =
  ...
  //
  // **ЧТО ИЗ ЭТОГО ПОЧИНЕНО 11.09, А ЧТО НЕТ — ЧИСЛОМ, А НЕ ВПЕЧАТЛЕНИЕМ.**
  // Имён, которые пишет клиент, ЧЕТЫРЕ; ветвь заведена ОДНОЙ. Молчат ещё
  // три, и у каждого свой номер находки — они названы ниже поимённо, а не
  // спрятаны под «прочее». Пятое имя, `ownerRestored`, в типе есть, а
  ...
  //                    личного вечера с 13.08). **ВЕТВЬ ЕСТЬ с 11.09,
  //                    N213 закрыта;**
  //   ownerRestored  — и вернул его же. **Писателя нет вовсе**, ветвь не
  //                    нужна: заводить её значило бы обслуживать поступок,
  //                    которого никто не совершает (I9);
  //   ownerFirm      — «всё точно», плашка состояния. **МОЛЧИТ, N214;**
  //   ownerDoubt     — «под вопросом», она же. **МОЛЧИТ, N215**, и это
  //                    молчание хуже прочих: окошко обещает вслух
  //                    «iştirakçılar xəbər tutacaq»;
  //   restored       — выход наверх из «под вопроса» (клиент пишет с
  //                    09.09). **МОЛЧИТ, N216.**
  //
  ...
  // строку в списке заведомо молчащих с номером находки. Заведи ветвь и
  // забудь список — сторож покраснеет, то есть изъятие не протухнет.
  | "ownerCancelled"
  | "ownerRestored"
  | "ownerFirm"
  | "ownerDoubt"
